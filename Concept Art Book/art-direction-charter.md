...
- A derelict contact may be misclassified as geology when its strata, accretion, burial, and repair fabric continue through the housing. Its artificial nature emerges through repeated state timing and precise material relationships, not a conventional console silhouette.
- Persistent signal surfaces use a quiet survival loop: dark hold, ember, bounded warning field, abstract non-text marks, interference, static, return to dark. Only the signal surface and one physically routed one-pixel light leak may pulse. The effect is a low hum or patient heartbeat, never an alarm strobe, magic glow, face, eye, or sentient expression.
- Signal-mineral studies must use original non-oval silhouettes and noncircular membranes. Preserve the emotional contradiction of an abandoned system still communicating, but never copy an external reference object's body, screen shape, glyphs, palette arrangement, environment, or animation frames. See the [frame-grounded Terminal reference](References/terminal-gif-inspiration/README.md).
