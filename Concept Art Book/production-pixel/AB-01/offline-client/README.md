...
The trace runs module → file → secret → request → response, then ends in an open split cap beyond the response. Repeated directional teeth point right, down, then left. Direction therefore survives grayscale and does not depend on hue.
...
- **Painted bounds:** approximately `x=5–61, y=5–61`; 57 x 57 logical pixels.
- **Hotspot:** retain `x=156, y=205, w=68, h=76`; ≥44 x 44 at native 1x.

The motif replaces only the physical overlay. It never enters the lower interface band or quiet footer rows `461–479`.
