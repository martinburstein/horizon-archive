...
print("Operator:", learner)`);
  ...
  await page.locator('[data-terminal-exercise="EX-L0603-OFFLINE-CAPSTONE"]').waitFor();await page.getByText("PASS · 15/15 objective rows ready",{exact:false}).waitFor();await page.setViewportSize({width:640,height:480});await page.screenshot({path:qaPath("capstone-readiness-primary-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});for(const id of Object.keys(referenceCapstonePrimary)){const a=referenceCapstonePrimary[id];await page.getByLabel("Capstone readiness decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Capstone readiness reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check capstone trace",exact:true}).click();await page.getByRole("status").getByText("2/2 · TRACE PASS · both dimensions confirmed.",{exact:true}).waitFor();await page.getByRole("button",{name:id==="P06"?"View primary result":"Next capstone boundary",exact:true}).click();}await page.getByRole("radio",{name:"medium",exact:true}).check();await page.getByRole("button",{name:"Acknowledge primary capstone",exact:true}).click();await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();await page.getByRole("button",{name:"Start Capstone Transfer",exact:true}).click();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("capstone-readiness-transfer-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});for(const id of Object.keys(referenceCapstoneTransfer)){const a=referenceCapstoneTransfer[id];await page.getByLabel("Capstone readiness decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Capstone readiness reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check capstone trace",exact:true}).click();await page.getByRole("button",{name:id==="T06"?"Begin closed-note defense":"Next capstone boundary",exact:true}).click();}await page.screenshot({path:qaPath("capstone-readiness-closed-note-qa.png"),fullPage:true});assertDistinctCaptures(["capstone-readiness-primary-qa.png","capstone-readiness-transfer-qa.png","capstone-readiness-closed-note-qa.png"]);const capstoneExplanation={client_flow:"separate endpoint identity deployment request result and errors",workload_direction:"analyze existing text recognize audio to text and synthesize text to audio",schema_provenance:"define schema preserve unsupported values as null and retain available evidence and provenance",prerequisite_readiness_safety:"require all 15 objectives ready closed fresh remediation routes and both forms before only recommending the next practice checkpoint with no exam guarantee or live authority"};for(const[d,v]of Object.entries(capstoneExplanation))await page.getByLabel(`Closed-note capstone ${d}`,{exact:true}).fill(v);const capstoneDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(capstoneDraft.includes("separate endpoint identity")||capstoneDraft.includes("no exam guarantee or live authority"))throw new Error("Capstone prose persisted");await page.getByRole("button",{name:"Check capstone defense",exact:true}).click();await page.getByRole("checkbox",{name:/produced this capstone explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge next-practice readiness",exact:true}).click();const capstoneContinue=page.getByRole("button",{name:"Continue",exact:true});await capstoneContinue.waitFor();if(!await capstoneContinue.evaluate(el=>el===document.activeElement))throw new Error("Capstone mastery did not focus Continue");await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const restoredCapstoneContinue=page.getByRole("button",{name:"Continue",exact:true});await restoredCapstoneContinue.waitFor();if(!await restoredCapstoneContinue.evaluate(el=>el===document.activeElement))throw new Error("Capstone reload did not focus Continue");await restoredCapstoneContinue.click();
  await page.locator('[data-terminal-exercise="EX-SIM01-MIXED"]').waitFor();await page.getByText("Untimed mode active · fully equivalent for mastery.",{exact:true}).waitFor();await page.setViewportSize({width:640,height:480});await page.screenshot({path:qaPath("sim-01-untimed-primary-qa.png"),fullPage:true});const timerToggle=page.getByRole("checkbox",{name:"Enable optional 25-minute diagnostic timer",exact:true});await timerToggle.check();await page.getByRole("timer").getByText("Diagnostic elapsed:",{exact:false}).waitFor();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("sim-01-timed-narrow-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});await timerToggle.uncheck();await page.getByText("Untimed mode active · fully equivalent for mastery.",{exact:true}).waitFor();await page.screenshot({path:qaPath("sim-01-untimed-scaled-qa.png"),fullPage:true});assertDistinctCaptures(["sim-01-untimed-primary-qa.png","sim-01-timed-narrow-qa.png","sim-01-untimed-scaled-qa.png"]);for(const id of Object.keys(referenceMixedSimulation)){const a=referenceMixedSimulation[id];await page.getByLabel("Mixed simulation decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Mixed simulation reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check simulation item",exact:true}).click();await page.getByRole("status").getByText("2/2 · ITEM PASS · both dimensions confirmed.",{exact:true}).waitFor();await page.getByRole("button",{name:id==="Q12"?"View strict result":"Next mixed item",exact:true}).click();}await page.getByText("24 / 24 dimensions",{exact:true}).waitFor();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge SIM-01 mastery",exact:true}).click();const simulationContinue=page.getByRole("button",{name:"Continue",exact:true});await simulationContinue.waitFor();if(!await simulationContinue.evaluate(el=>el===document.activeElement))throw new Error("SIM-01 mastery did not focus Continue");const simulationMastery=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).mixedSimulationEvidence,{key:saveKey});if(simulationMastery?.masteryStatus!=="mastered"||Object.keys(simulationMastery.dimensionCorrectness||{}).length!==12)throw new Error(`SIM-01 mastery incomplete ${JSON.stringify(simulationMastery)}`);await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const restoredSimulationContinue=page.getByRole("button",{name:"Continue",exact:true});await restoredSimulationContinue.waitFor();if(!await restoredSimulationContinue.evaluate(el=>el===document.activeElement))throw new Error("SIM-01 reload did not focus Continue");await restoredSimulationContinue.click();
