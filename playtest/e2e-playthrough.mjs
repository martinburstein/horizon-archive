...
print("Operator:", learner)`);
  ...
  await page.getByText("FULLY OFFLINE", { exact: false }).waitFor(); await page.getByText("Media-equivalent scenario text", { exact: false }).waitFor();
  ...
  for (const id of Object.keys(referenceExtractionPrimary)) { const answer = referenceExtractionPrimary[id]; await page.getByLabel("Extraction decision", { exact: true }).selectOption(answer.decision); await page.getByLabel("Extraction reason", { exact: true }).selectOption(answer.reason); await page.getByRole("button", { name: "Check extraction choice", exact: true }).click(); await page.getByText("Choice confirmed", { exact: false }).waitFor(); await page.getByRole("button", { name: id === "P06" ? "View primary result" : "Next scenario", exact: true }).click(); }
  ...
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); await page.getByRole("button", { name: "Start Extraction Transfer", exact: true }).click();
  await page.getByLabel("Extraction decision", { exact: true }).selectOption("summarization"); await page.getByLabel("Extraction reason", { exact: true }).selectOption("a_summary_is_the_same_as_named_fields"); await page.getByRole("button", { name: "Check extraction choice", exact: true }).click(); await page.getByRole("status").getByText("0/2", { exact: false }).waitFor();
  ...
  for (const id of Object.keys(referenceExtractionTransfer)) { const answer = referenceExtractionTransfer[id]; await page.getByLabel("Extraction decision", { exact: true }).selectOption(answer.decision); await page.getByLabel("Extraction reason", { exact: true }).selectOption(answer.reason); await page.getByRole("button", { name: "Check extraction choice", exact: true }).click(); await page.getByText("Choice confirmed", { exact: false }).waitFor(); await page.getByRole("button", { name: id === "T06" ? "Begin closed-note explanation" : "Next scenario", exact: true }).click(); }
  for (const d of ["modality", "schema", "missing_value", "evidence_review"]) await page.getByLabel(`Closed-note extraction ${d}`, { exact: true }).fill("wrong"); await page.getByRole("button", { name: "Check extraction explanation", exact: true }).click(); await page.getByRole("status").getByText("0/4", { exact: false }).waitFor(); await page.screenshot({ path: qaPath("extraction-workloads-closed-note-qa.png"), fullPage: true }); assertDistinctCaptures(["extraction-workloads-primary-qa.png", "extraction-workloads-transfer-remediation-qa.png", "extraction-workloads-closed-note-qa.png"]);
  ...
  const extractionExplanationDraft = await page.evaluate(({ key }) => localStorage.getItem(key), { key: saveKey }); if (extractionExplanationDraft.includes("preserve null or missing") || extractionExplanationDraft.includes("retain provenance")) throw new Error("Extraction explanation leaked into storage"); await page.getByRole("button", { name: "Check extraction explanation", exact: true }).click(); await page.getByText("Complete extraction explanation confirmed", { exact: false }).waitFor(); await page.getByRole("checkbox", { name: "I produced this extraction explanation myself without notes.", exact: true }).check(); await page.getByRole("radio", { name: "high", exact: true }).check(); await page.getByRole("button", { name: "Acknowledge strict mastery", exact: true }).click();
  ...
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredVisualContinue = page.getByRole("button", { name: "Continue", exact: true }); await restoredVisualContinue.waitFor(); if (!await restoredVisualContinue.evaluate((element) => element === document.activeElement)) throw new Error("Sanitized Extraction mastery reload did not restore focus to Continue");
