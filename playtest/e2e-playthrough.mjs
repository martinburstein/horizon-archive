...
print("Operator:", learner)`);
  ...
  await severedRelaySpine.waitFor();
  if (await severedRelaySpine.getAttribute("data-severed-relay-spine-state") !== "complete") throw new Error("Host 08 did not expose read-only completion after Client Bridge mastery");
  await severedRelaySpine.click();
  if (await page.locator('[data-terminal-exercise="EX-L0303-CLIENT-BRIDGE"]').count()) throw new Error("Completed Host 08 reopened Client Bridge");
