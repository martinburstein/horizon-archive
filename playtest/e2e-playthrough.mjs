...
print("Operator:", learner)`);
  ...
  await page.getByText("SYSTEM // Expedition work remains unfinished and can resume from a clean boundary. SCENE // Water and branches remain unchanged.", { exact: true }).waitFor();
  ...
  const strandedLensCradle = page.locator('[data-hotspot-id="stranded-lens-cradle"]');
  const useStrandedLensCradle = async () => {
    await page.getByRole("button", { name: "USE", exact: true }).click();
    await strandedLensCradle.click();
  };
  await strandedLensCradle.waitFor();
  await page.waitForFunction(() => document.activeElement?.dataset.hotspotId === "stranded-lens-cradle");
  sixfoldFocus.mastery = await strandedLensCradle.evaluate((element) => document.activeElement === element);
  ...
  sixfoldFocus.nextContinuation = await strandedLensCradle.isVisible();

  await useStrandedLensCradle();
  ...
  const sedimentAbacus = page.locator('[data-hotspot-id="sediment-abacus"]');
  const useSedimentAbacus = async () => {
    await page.getByRole("button", { name: "USE", exact: true }).click();
    await sedimentAbacus.click();
  };
  await sedimentAbacus.waitFor();
  await page.waitForFunction(() => document.activeElement?.dataset.hotspotId === "sediment-abacus");
  await useSedimentAbacus();
  ...
  await page.getByRole("button", { name: "Acknowledge strict mastery", exact: true }).click();
  await teacherSpeaker.getByText("901 TEACHER // SOURCE-GROUNDED COURSE", { exact: true }).waitFor();
  const structuredContinue = sedimentAbacus;
  ...
  if (await severedRelaySpine.getAttribute("data-severed-relay-spine-state") !== "available") throw new Error("Host 08 was not available after Control Flow mastery");
  ...
  await page.getByRole("button", { name: "Resume signal" }).click();
  await page.getByRole("button", { name: "USE", exact: true }).click();
  await page.locator('[data-hotspot-id="severed-relay-spine"]').click();
  ...
  await page.getByRole("button", { name: "Acknowledge strict mastery", exact: true }).click();
  await teacherSpeaker.getByText("901 TEACHER // SOURCE-GROUNDED COURSE", { exact: true }).waitFor();
  const floodedChoir = page.locator('[data-hotspot-id="flooded-choir"]');
  await floodedChoir.waitFor();
  if (await floodedChoir.getAttribute("data-flooded-choir-state") !== "available") throw new Error("Host 09 was not available after Client Bridge mastery");
  if (!await floodedChoir.evaluate((element) => element === document.activeElement)) throw new Error("Client Bridge mastery did not move focus to Host 09");
  ...
  await page.reload();
  await page.getByRole("button", { name: "Resume signal" }).click();
  await page.getByRole("button", { name: "USE", exact: true }).click(); await floodedChoir.click();
  ...
  await teacherSpeaker.getByText("901 TEACHER // SOURCE-GROUNDED COURSE", { exact: true }).waitFor();
  await floodedChoir.waitFor();
  if (!await floodedChoir.evaluate((element) => element === document.activeElement)) throw new Error("Text Analysis mastery did not return focus to Host 09");
  ...
  await page.getByRole("button", { name: "Acknowledge strict mastery", exact: true }).click();
  await teacherSpeaker.getByText("901 TEACHER // SOURCE-GROUNDED COURSE", { exact: true }).waitFor();
  const blindCameraWell = page.locator('[data-hotspot-id="blind-camera-well"]'); await blindCameraWell.waitFor(); if (!await blindCameraWell.evaluate((el) => el === document.activeElement)) throw new Error("Speech mastery did not focus Host 10");
  ...
  await blindCameraWell.waitFor();
  if (await blindCameraWell.getAttribute("data-blind-camera-well-state") !== "available") throw new Error("Host 10 was not available after Host 09 mastery");
  if (await page.locator('[data-terminal-exercise="EX-L0401-TEXT-ANALYSIS"], [data-terminal-exercise="EX-L0402-SPEECH-WORKLOADS"]').count()) throw new Error("Host 09 completion left an owned lesson open");
  await page.reload();
  await page.getByRole("button", { name: "Resume signal" }).click();
  const restoredSpeechContinue = page.locator('[data-hotspot-id="blind-camera-well"]');
  await restoredSpeechContinue.waitFor();
  if (!await restoredSpeechContinue.evaluate((element) => element === document.activeElement)) throw new Error("Sanitized Speech mastery reload did not restore focus to Host 10");
  ...
  await page.getByRole("radio", { name: "medium", exact: true }).check();
  await page.getByRole("button", { name: "Acknowledge primary form", exact: true }).click();
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); await page.getByRole("button", { name: "USE", exact: true }).click(); await page.locator('[data-hotspot-id="blind-camera-well"]').click();
  ...
  await page.getByRole("button", { name: "Exit Visual Workloads", exact: true }).click(); await page.locator('[data-hotspot-id="blind-camera-well"]').click();
  ...
  const visualContinue = page.locator('[data-hotspot-id="blind-camera-well"]'); await visualContinue.waitFor(); if (!await visualContinue.evaluate((element) => element === document.activeElement)) throw new Error("Visual mastery did not focus Host 10");
  ...
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); await page.getByRole("button", { name: "USE", exact: true }).click(); await page.locator('[data-hotspot-id="blind-camera-well"]').click(); await assertExtractionContinuity(page, "transfer");
  ...
  await page.getByRole("button", { name: "Exit Extraction Workloads", exact: true }).click(); await page.locator('[data-hotspot-id="blind-camera-well"]').click(); if (await page.getByLabel("Extraction reason", { exact: true }).inputValue() !== "a_summary_is_the_same_as_named_fields") throw new Error("Extraction transfer reset after close/reopen");
  ...
  for (const [d, value] of Object.entries(extractionExplanation)) await page.getByLabel(`Closed-note extraction ${d}`, { exact: true }).fill(value); await page.getByRole("button", { name: "Exit Extraction Workloads", exact: true }).click(); await page.locator('[data-hotspot-id="blind-camera-well"]').click(); if (await page.getByLabel("Closed-note extraction missing_value", { exact: true }).inputValue() !== extractionExplanation.missing_value) throw new Error("Extraction explanation reset after close/reopen");
  ...
  const extractionContinue = page.locator('[data-hotspot-id="borrowed-light-wall"]'); await extractionContinue.waitFor(); if (!await extractionContinue.evaluate((element) => element === document.activeElement)) throw new Error("Extraction mastery did not focus Host 11");
  ...
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredExtractionContinue = page.locator('[data-hotspot-id="borrowed-light-wall"]'); await restoredExtractionContinue.waitFor(); if (!await restoredExtractionContinue.evaluate((element) => element === document.activeElement)) throw new Error("Sanitized Extraction mastery reload did not restore focus to Host 11");
  await page.getByRole("button", { name: "USE", exact: true }).click(); await restoredExtractionContinue.click();
  ...
  await page.getByRole("radio", { name: "medium", exact: true }).check(); await page.getByRole("button", { name: "Acknowledge primary form", exact: true }).click();
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); await page.getByRole("button", { name: "USE", exact: true }).click(); await page.locator('[data-hotspot-id="borrowed-light-wall"]').click(); await assertPortalContinuity(page, "transfer");
  ...
  await page.getByRole("button", { name: "Exit Portal Orientation", exact: true }).click(); await page.locator('[data-hotspot-id="borrowed-light-wall"]').click(); if (await page.getByLabel("Portal reason", { exact: true }).inputValue() !== "portal_visibility_proves_correct_permissions") throw new Error("Portal transfer reset after close/reopen");
  ...
  for (const [dimension, value] of Object.entries(portalExplanation)) await page.getByLabel(`Closed-note portal ${dimension}`, { exact: true }).fill(value); await page.getByRole("button", { name: "Exit Portal Orientation", exact: true }).click(); await page.locator('[data-hotspot-id="borrowed-light-wall"]').click(); if (await page.getByLabel("Closed-note portal cleanup", { exact: true }).inputValue() !== portalExplanation.cleanup) throw new Error("Portal explanation reset after close/reopen");
  ...
  const portalContinue = page.locator('[data-hotspot-id="borrowed-light-wall"]'); await portalContinue.waitFor(); if (!await portalContinue.evaluate((element) => element === document.activeElement)) throw new Error("Portal mastery did not focus Host 11");
  ...
  await page.locator('[data-hotspot-id="borrowed-light-wall"]').click(); await page.locator('[data-terminal-exercise="EX-L0502-PROMPT-LAYERS"]').waitFor(); await assertPromptContinuity(page, "primary");
  ...
  await page.setViewportSize({ width: 1600, height: 900 });
  for(const[id,answer]of Object.entries(referenceSdkRoutePrimary)){await page.getByLabel("SDK route",{exact:true}).selectOption(answer.route);await page.getByLabel("Reason for SDK route",{exact:true}).selectOption(answer.reason);await page.getByRole("button",{name:"Check route and reason",exact:true}).click();await page.getByText("ROUTE + REASON PASS",{exact:false}).waitFor();await page.getByRole("button",{name:id==="P08"?"View form result":"Next scenario",exact:true}).click();}await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Begin fresh transfer later",exact:true}).click();const sdkTransferContinue=page.locator('[data-hotspot-id="drowned-switchyard"]');await sdkTransferContinue.waitFor();if(!await sdkTransferContinue.evaluate(el=>el===document.activeElement))throw new Error("SDK Route primary completion did not focus the selected Host 12 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await sdkTransferContinue.click();for(const[id,answer]of Object.entries(referenceSdkRouteTransfer)){await page.getByLabel("SDK route",{exact:true}).selectOption(answer.route);await page.getByLabel("Reason for SDK route",{exact:true}).selectOption(answer.reason);await page.getByRole("button",{name:"Check route and reason",exact:true}).click();await page.getByText("ROUTE + REASON PASS",{exact:false}).waitFor();await page.getByRole("button",{name:id==="T08"?"View form result":"Next scenario",exact:true}).click();}await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge route mastery",exact:true}).click();const singleAgentAfterSdk=page.locator('[data-hotspot-id="drowned-switchyard"]');await singleAgentAfterSdk.waitFor();if(!await singleAgentAfterSdk.evaluate(el=>el===document.activeElement))throw new Error("SDK Route mastery did not focus the selected Host 12 boundary");const sdkEvidence=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).sdkRouteEvidence,{key:saveKey});if(sdkEvidence?.masteryStatus!=="mastered"||sdkEvidence?.attemptCount!==17||sdkEvidence?.remediationAttemptCount!==2)throw new Error(`SDK route mastery incomplete ${JSON.stringify(sdkEvidence)}`);if(["response","endpoint","credential","deploymentName","request","clickTime","keySequence"].some(key=>key in sdkEvidence))throw new Error("SDK route private or dexterity data persisted");
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredClientBoundaryContinue = page.locator('[data-hotspot-id="drowned-switchyard"]'); await restoredClientBoundaryContinue.waitFor(); if(!await restoredClientBoundaryContinue.evaluate(el=>el===document.activeElement))throw new Error("Sanitized SDK route mastery reload did not restore focus to the selected Host 12 boundary"); await page.getByRole("button", { name: "USE", exact: true }).click(); await restoredClientBoundaryContinue.click();
  ...
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const singleAgentTransferContinue=page.locator('[data-hotspot-id="drowned-switchyard"]');await singleAgentTransferContinue.waitFor();if(!await singleAgentTransferContinue.evaluate(el=>el===document.activeElement))throw new Error("Single Agent primary reload did not focus the selected Host 12 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await singleAgentTransferContinue.click();await assertSingleAgentContinuity(page,"transfer");await page.getByLabel("Single agent decision",{exact:true}).selectOption("always_use_agent");await page.getByLabel("Single agent reason",{exact:true}).selectOption("every_prompt_requires_agent_orchestration");await page.getByRole("button",{name:"Check single agent",exact:true}).click();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("single-agent-transfer-remediation-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});await page.getByRole("button",{name:"Exit Single Agent",exact:true}).click();const singleAgentTransferReopen=page.locator('[data-hotspot-id="drowned-switchyard"]');await singleAgentTransferReopen.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="drowned-switchyard");if(!await singleAgentTransferReopen.evaluate(el=>el===document.activeElement))throw new Error("Single Agent transfer close did not restore focus to the selected Host 12 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await singleAgentTransferReopen.click();if(await page.getByLabel("Single agent reason",{exact:true}).inputValue()!=="every_prompt_requires_agent_orchestration")throw new Error("Single Agent transfer reset after close/reopen");const singleAgentDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(singleAgentDraft.includes("every_prompt_requires_agent_orchestration")||singleAgentDraft.includes("always_use_agent"))throw new Error("Single Agent choices persisted");
  ...
  const singleAgentExplanation={fit_instructions:"use agent only for goal tools orchestration with stable instructions",least_privilege:"attach only required tools and tool capability is not permission",failure_safety:"test edge injection failure and denied paths never fabricate success",client_flow:"use agent identifier submit input then read result or error"};for(const[dimension,value]of Object.entries(singleAgentExplanation))await page.getByLabel(`Closed-note single agent ${dimension}`,{exact:true}).fill(value);await page.getByRole("button",{name:"Exit Single Agent",exact:true}).click();const singleAgentExplanationReopen=page.locator('[data-hotspot-id="drowned-switchyard"]');await singleAgentExplanationReopen.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="drowned-switchyard");if(!await singleAgentExplanationReopen.evaluate(el=>el===document.activeElement))throw new Error("Single Agent closed-note close did not restore focus to the selected Host 12 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await singleAgentExplanationReopen.click();if(await page.getByLabel("Closed-note single agent failure_safety",{exact:true}).inputValue()!==singleAgentExplanation.failure_safety)throw new Error("Single Agent explanation reset after close/reopen");const singleAgentExplanationDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(singleAgentExplanationDraft.includes("never fabricate success")||singleAgentExplanationDraft.includes("tool capability is not permission"))throw new Error("Single Agent explanation persisted");await page.getByRole("button",{name:"Check single-agent explanation",exact:true}).click();await page.getByRole("status").getByText("4/4 · PASS",{exact:true}).waitFor();await page.getByRole("checkbox",{name:/produced this single-agent explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const singleAgentContinue=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await singleAgentContinue.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await singleAgentContinue.evaluate(element=>element===document.activeElement))throw new Error("Single Agent mastery did not focus the selected Host 13 boundary");const singleAgentMastery=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).singleAgentEvidence,{key:saveKey});if(singleAgentMastery?.masteryStatus!=="mastered"||singleAgentMastery?.attemptCount!==16)throw new Error(`Single Agent mastery incomplete ${JSON.stringify(singleAgentMastery)}`);if(["agentInstructions","toolPayload","toolResult","agentId","endpoint","credential","conversationText","externalActionRequest","freeText","response","choices"].some(key=>key in singleAgentMastery))throw new Error("Single Agent private data persisted");await page.getByRole("button",{name:"USE",exact:true}).click();await singleAgentContinue.click();
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const restoredSingleAgentContinue=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await restoredSingleAgentContinue.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await restoredSingleAgentContinue.evaluate(element=>element===document.activeElement))throw new Error("Single Agent mastery reload did not focus the selected Host 13 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await restoredSingleAgentContinue.click();
  ...
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const textSpeechTransferContinue=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await textSpeechTransferContinue.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await textSpeechTransferContinue.evaluate(element=>element===document.activeElement))throw new Error("Text/Speech primary reload did not focus the selected Host 13 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await textSpeechTransferContinue.click();await assertTextSpeechPatternContinuity(page,"transfer");await page.getByLabel("Text speech pattern decision",{exact:true}).selectOption("generate_new_text");await page.getByLabel("Text speech pattern reason",{exact:true}).selectOption("generation_and_analysis_are_interchangeable");await page.getByRole("button",{name:"Check text/speech pattern",exact:true}).click();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("text-speech-patterns-transfer-remediation-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});await page.getByRole("button",{name:"Exit Text and Speech Patterns",exact:true}).click();const textSpeechTransferReopen=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await textSpeechTransferReopen.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await textSpeechTransferReopen.evaluate(element=>element===document.activeElement))throw new Error("Text/Speech transfer close did not restore focus to the selected Host 13 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await textSpeechTransferReopen.click();if(await page.getByLabel("Text speech pattern reason",{exact:true}).inputValue()!=="generation_and_analysis_are_interchangeable")throw new Error("Text/Speech transfer reset");const textSpeechDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(textSpeechDraft.includes("generation_and_analysis_are_interchangeable")||textSpeechDraft.includes("generate_new_text"))throw new Error("Text/Speech choices persisted");
  ...
  const textSpeechExplanation={capability_direction:"choose capability from input and required output including recognition audio to text and synthesis text to audio",configuration_payload:"keep endpoint identity and payload separate and never hardcode credentials",result_cancellation:"inspect per item result error and cancellation and never fabricate content",simulation_authority:"offline output proves no live readiness and never authorizes disclosure or external action"};for(const[dimension,value]of Object.entries(textSpeechExplanation))await page.getByLabel(`Closed-note text speech ${dimension}`,{exact:true}).fill(value);await page.getByRole("button",{name:"Exit Text and Speech Patterns",exact:true}).click();const textSpeechBoundary=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await textSpeechBoundary.waitFor();await page.getByRole("button",{name:"USE",exact:true}).click();await textSpeechBoundary.click();if(await page.getByLabel("Closed-note text speech result_cancellation",{exact:true}).inputValue()!==textSpeechExplanation.result_cancellation)throw new Error("Text/Speech explanation reset");const textSpeechExplanationDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(textSpeechExplanationDraft.includes("never fabricate content")||textSpeechExplanationDraft.includes("never hardcode credentials"))throw new Error("Text/Speech explanation persisted");await page.getByRole("button",{name:"Check text/speech explanation",exact:true}).click();await page.getByText("4/4 · PASS",{exact:true}).waitFor();await page.getByRole("checkbox",{name:/produced this text\/speech explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const textSpeechContinue=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await textSpeechContinue.waitFor();if(!await textSpeechContinue.evaluate(element=>element===document.activeElement))throw new Error("Text/Speech mastery did not focus the selected Host 13 boundary");const textSpeechMastery=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).textSpeechPatternEvidence,{key:saveKey});if(textSpeechMastery?.masteryStatus!=="mastered"||textSpeechMastery?.attemptCount!==16)throw new Error(`Text/Speech mastery incomplete ${JSON.stringify(textSpeechMastery)}`);if(["inputText","audioBytes","audioPath","transcript","generatedAudio","endpoint","credential","serviceResponse","externalActionRequest","response","choices","freeText"].some(key=>key in textSpeechMastery))throw new Error("Text/Speech private data persisted");await page.getByRole("button",{name:"USE",exact:true}).click();await textSpeechContinue.click();
  ...
  for(const id of Object.keys(referenceVisualPatternPrimary)){const a=referenceVisualPatternPrimary[id];await page.getByLabel("Visual pattern decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Visual pattern reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check visual pattern",exact:true}).click();await page.getByText("CHOICE PASS",{exact:false}).waitFor();await page.getByRole("button",{name:id==="P06"?"View primary result":"Next scenario",exact:true}).click();}await page.getByRole("radio",{name:"medium",exact:true}).check();await page.getByRole("button",{name:"Acknowledge primary form",exact:true}).click();await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const visualPatternTransferContinue=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await visualPatternTransferContinue.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await visualPatternTransferContinue.evaluate(element=>element===document.activeElement))throw new Error("Visual Pattern primary reload did not focus the selected Host 13 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await visualPatternTransferContinue.click();await assertVisualPatternContinuity(page,"transfer");await page.getByLabel("Visual pattern decision",{exact:true}).selectOption("generate_new_image");await page.getByLabel("Visual pattern reason",{exact:true}).selectOption("analysis_and_generation_are_interchangeable");await page.getByRole("button",{name:"Check visual pattern",exact:true}).click();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("visual-patterns-transfer-remediation-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});await page.getByRole("button",{name:"Exit Visual Patterns",exact:true}).click();const visualPatternTransferReopen=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await visualPatternTransferReopen.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await visualPatternTransferReopen.evaluate(element=>element===document.activeElement))throw new Error("Visual Pattern transfer close did not restore focus to the selected Host 13 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await visualPatternTransferReopen.click();if(await page.getByLabel("Visual pattern reason",{exact:true}).inputValue()!=="analysis_and_generation_are_interchangeable")throw new Error("Visual Pattern transfer reset");const visualPatternDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(visualPatternDraft.includes("analysis_and_generation_are_interchangeable")||visualPatternDraft.includes("generate_new_image"))throw new Error("Visual Pattern choices persisted");
  ...
  const visualPatternExplanation={capability_media:"choose analysis for existing pixels multimodal for image plus text and generation for new media",request_validation:"validate media bytes type request and deployed capability before processing",result_provenance:"parse operation specific result shape and label generated content with source model prompt and time",simulation_authority:"offline output proves no live readiness and never authorizes publication or deletion"};for(const[d,v]of Object.entries(visualPatternExplanation))await page.getByLabel(`Closed-note visual pattern ${d}`,{exact:true}).fill(v);await page.getByRole("button",{name:"Exit Visual Patterns",exact:true}).click();const visualPatternExplanationReopen=page.locator('[data-hotspot-id="tidal-pattern-loom"]');await visualPatternExplanationReopen.waitFor();await page.waitForFunction(()=>document.activeElement?.getAttribute("data-hotspot-id")==="tidal-pattern-loom");if(!await visualPatternExplanationReopen.evaluate(element=>element===document.activeElement))throw new Error("Visual Pattern closed-note close did not restore focus to the selected Host 13 boundary");await page.getByRole("button",{name:"USE",exact:true}).click();await visualPatternExplanationReopen.click();if(await page.getByLabel("Closed-note visual pattern result_provenance",{exact:true}).inputValue()!==visualPatternExplanation.result_provenance)throw new Error("Visual Pattern explanation reset");const visualPatternExplanationDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(visualPatternExplanationDraft.includes("label generated content")||visualPatternExplanationDraft.includes("never authorizes publication"))throw new Error("Visual Pattern explanation persisted");await page.getByRole("button",{name:"Check visual-pattern explanation",exact:true}).click();await page.getByText("4/4 · PASS",{exact:true}).waitFor();await page.getByRole("checkbox",{name:/produced this visual-pattern explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const visualPatternContinue=getHost14OwnedControl(page,"Start Objective Ledger");await waitForOwnedControlFocus(page,"waterline-ledger","Start Objective Ledger");const visualPatternMastery=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).visualPatternEvidence,{key:saveKey});if(visualPatternMastery?.masteryStatus!=="mastered"||visualPatternMastery?.attemptCount!==16)throw new Error(`Visual Pattern mastery incomplete ${JSON.stringify(visualPatternMastery)}`);if(["mediaBytes","mediaPath","visualDescription","generationPrompt","generatedMedia","endpoint","credential","serviceResponse","externalActionRequest","response","choices","freeText"].some(k=>k in visualPatternMastery))throw new Error("Visual Pattern private data persisted");await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const restoredVisualPatternContinue=getHost14OwnedControl(page,"Start Objective Ledger");await waitForOwnedControlFocus(page,"waterline-ledger","Start Objective Ledger");await openHost14OwnedLesson(page,"Start Objective Ledger");
  ...
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();await openHost14OwnedLesson(page,"Start Objective Ledger Transfer");await assertObjectiveLedgerContinuity(page,"transfer");await page.getByLabel("Objective ledger decision",{exact:true}).selectOption("AI901-D1-O2");await page.getByLabel("Objective ledger reason",{exact:true}).selectOption("confidence_or_domain_score_proves_this_objective");await page.getByRole("button",{name:"Check objective evidence",exact:true}).click();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("objective-ledger-transfer-remediation-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});await page.getByRole("button",{name:"Exit Objective Ledger",exact:true}).click();await openHost14OwnedLesson(page,"Resume Objective Ledger");if(await page.getByLabel("Objective ledger reason",{exact:true}).inputValue()!=="confidence_or_domain_score_proves_this_objective")throw new Error("Objective Ledger transfer reset after close/reopen");const objectiveLedgerDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(objectiveLedgerDraft.includes("confidence_or_domain_score_proves_this_objective"))throw new Error("Objective Ledger working choices persisted");
  ...
  const simulationFocus = page.locator('[data-hotspot-id="crown-counterpoise"]');
  await waitForOwnedControlFocus(
    page,
    "crown-counterpoise",
    "Continue to the next survey site",
  );
  if (
    !(await simulationFocus.evaluate((el) => el === document.activeElement))
  )
    throw new Error("Mixed block completion did not focus the Host 15 final-confidence boundary");
  ...
  const restoredSimulationFocus = page.locator('[data-hotspot-id="crown-counterpoise"]');
  await waitForOwnedControlFocus(
    page,
    "crown-counterpoise",
    "Continue to the next survey site",
  ...
  if (
    !(await restoredSimulationFocus.evaluate(
      (el) => el === document.activeElement,
    ))
  )
    throw new Error("Mixed block reload did not focus the Host 15 final-confidence boundary");
  ...
  await page.getByRole("button", { name: "use Witness Spine", exact: true }).click();
  await page.locator('[data-terminal-exercise="EX-L0507-EVIDENCE-PACKET"]').waitFor();
...
async function completeFinalConfidence(page) {
  await openHost15OwnedLesson(page, "Open Final Confidence Gate");
...
async function openHost14OwnedLesson(page, genericName) {
  ...
  await waitForOwnedControlAvailable(page, "waterline-ledger", genericName);
...
function getHost14OwnedControl(page, genericName) {
  return page.locator('[data-hotspot-id="waterline-ledger"]').or(
    page.getByRole("button", { name: genericName, exact: true }),
  );
}

function getHost15OwnedControl(page, genericName) {
  return page.locator('[data-hotspot-id="crown-counterpoise"]').or(
    page.getByRole("button", { name: genericName, exact: true }),
  );
}

async function waitForOwnedControlFocus(page, hotspotId, genericName) {
  await page.waitForFunction(
    ({ hotspotId: expectedHotspotId, genericName: expectedGenericName }) => {
      const active = document.activeElement;
      return active?.getAttribute("data-hotspot-id") === expectedHotspotId ||
        active?.textContent?.trim() === expectedGenericName;
    },
    { hotspotId, genericName },
  );
}

async function waitForOwnedControlAvailable(page, hotspotId, genericName) {
  await page.waitForFunction(
    ({ hotspotId: expectedHotspotId, genericName: expectedGenericName }) => {
      const visible = (element) => Boolean(
        element && (element.offsetWidth || element.offsetHeight || element.getClientRects().length),
      );
      if (visible(document.querySelector(`[data-hotspot-id="${expectedHotspotId}"]`))) return true;
      return [...document.querySelectorAll("button")].some(
        (button) => visible(button) && button.textContent?.trim() === expectedGenericName,
      );
    },
    { hotspotId, genericName },
  );
...
async function openHost15OwnedLesson(page, genericName) {
  const nativeBoundary = page.locator('[data-hotspot-id="crown-counterpoise"]');
  await waitForOwnedControlAvailable(page, "crown-counterpoise", genericName);
  if (await nativeBoundary.count() && await nativeBoundary.isVisible()) {
    await page.getByRole("button", { name: "USE", exact: true }).click();
    await nativeBoundary.click();
    return "native";
...
async function captureWitnessScene(page, path) {
  await page.getByRole("button", { name: "USE", exact: true }).click();
  const terminal = page.getByRole("button", { name: "use Witness Spine", exact: true });
...
async function assertWitnessHotspotAlignment(page, viewportLabel) {
  ...
  const metrics = await page.evaluate((narrow) => {
    ...
    const measure = (element, source, frameSpace = false) => {
      const rect = element.getBoundingClientRect();
      const expected = frameSpace ? {
        left: frameRect.left + frameRect.width * source.left,
        top: frameRect.top + frameRect.height * source.top,
        right: frameRect.left + frameRect.width * source.right,
        bottom: frameRect.top + frameRect.height * source.bottom,
      } : {
    ...
    return {
      ...
      naturalWidth: image.naturalWidth,
      naturalHeight: image.naturalHeight,
      terminal: measure(terminal, narrow
        ? { left: 0, top: .42, right: .30, bottom: .86 }
        : { left: 150 / 1920, top: 350 / 1080, right: 480 / 1920, bottom: 800 / 1080 }, narrow),
      automaton: measure(automaton, narrow
        ? { left: .46, top: .17, right: 1, bottom: .83 }
        : { left: 760 / 1920, top: 150 / 1080, right: 1880 / 1920, bottom: 870 / 1080 }, narrow),
    };
  }, viewportLabel === "320px narrow");

  if (!metrics) throw new Error(`Witness geometry unavailable at ${viewportLabel}`);
  if (!metrics.src.includes("host16-environment-master-v1")) throw new Error(`Wrong Witness asset at ${viewportLabel}: ${metrics.src}`);
  if (!/silent fallen alien assembly/i.test(metrics.alt) || !/separate three-aperture coupling/i.test(metrics.alt) || !/open route on the left/i.test(metrics.alt)) {
    throw new Error(`Witness alt text does not distinguish both objects: ${metrics.alt}`);
  }
  if (metrics.naturalWidth !== 1920 || metrics.naturalHeight !== 1080) throw new Error(`Unexpected Witness asset dimensions: ${metrics.naturalWidth}x${metrics.naturalHeight}`);
