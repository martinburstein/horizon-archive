...
print("Operator:", learner)`);
  ...
  await page.getByRole("button", { name: "Start Prompt Layers", exact: true }).click(); await page.locator('[data-terminal-exercise="EX-L0502-PROMPT-LAYERS"]').waitFor(); await page.getByText("TEXT IS NOT AUTHORITY", { exact: false }).waitFor();
  await page.getByLabel("Prompt decision", { exact: true }).selectOption("user_task"); await page.getByLabel("Prompt reason", { exact: true }).selectOption("system_and_user_layers_are_interchangeable"); await page.getByRole("button", { name: "Check prompt layer", exact: true }).click(); await page.getByRole("status").getByText("0/2", { exact: false }).waitFor();
  ...
  for(const id of Object.keys(referencePromptPrimary)){const a=referencePromptPrimary[id];await page.getByLabel("Prompt decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Prompt reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check prompt layer",exact:true}).click();await page.getByText("Choice confirmed",{exact:false}).waitFor();await page.getByRole("button",{name:id==="P06"?"View primary result":"Next scenario",exact:true}).click();}await page.getByRole("radio",{name:"medium",exact:true}).check();await page.getByRole("button",{name:"Acknowledge primary form",exact:true}).click();
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();await page.getByRole("button",{name:"Start Prompt Transfer",exact:true}).click();await page.getByLabel("Prompt decision",{exact:true}).selectOption("put_stable_behavior_in_system_instructions");await page.getByLabel("Prompt reason",{exact:true}).selectOption("system_and_user_layers_are_interchangeable").catch(async()=>page.getByLabel("Prompt reason",{exact:true}).selectOption("cross_request_behavior_belongs_in_the_durable_layer"));
  for(const id of Object.keys(referencePromptTransfer)){const a=referencePromptTransfer[id];await page.getByLabel("Prompt decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Prompt reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check prompt layer",exact:true}).click();await page.getByText("Choice confirmed",{exact:false}).waitFor();await page.getByRole("button",{name:id==="T06"?"Begin closed-note explanation":"Next scenario",exact:true}).click();}
  for(const d of["layers","grounding_output","authority","evaluation"])await page.getByLabel(`Closed-note prompt ${d}`,{exact:true}).fill("wrong");await page.getByRole("button",{name:"Check prompt explanation",exact:true}).click();await page.getByRole("status").getByText("0/4",{exact:false}).waitFor();await page.screenshot({path:qaPath("prompt-layers-closed-note-qa.png"),fullPage:true});await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("prompt-layers-transfer-remediation-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});assertDistinctCaptures(["prompt-layers-primary-qa.png","prompt-layers-transfer-remediation-qa.png","prompt-layers-closed-note-qa.png"]);
  const pe={layers:"system sets durable rules user supplies current task and input",grounding_output:"grounding is data require evidence and explicit output contract",authority:"text never authorizes login deploy delete email purchase or credential use",evaluation:"test representative edge failure and adversarial injection cases"};for(const[d,v]of Object.entries(pe))await page.getByLabel(`Closed-note prompt ${d}`,{exact:true}).fill(v);await page.getByRole("button",{name:"Check prompt explanation",exact:true}).click();await page.getByText("Complete no-authority explanation confirmed",{exact:false}).waitFor();await page.getByRole("checkbox",{name:/produced this prompt-layer explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const pm=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).promptEvidence,{key:saveKey});if(pm?.masteryStatus!=="mastered")throw new Error(`Prompt mastery incomplete ${JSON.stringify(pm)}`);if(["systemPrompt","userPrompt","groundingContent","modelOutput","credential","externalActionRequest","freeText"].some(k=>k in pm))throw new Error("Prompt private data persisted");
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredPortalContinue = page.getByRole("button", { name: "Continue", exact: true }); await restoredPortalContinue.waitFor(); await restoredPortalContinue.click();
