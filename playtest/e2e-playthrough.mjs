...
print("Operator:", learner)`);
  ...
  const textSpeechExplanation={capability_direction:"choose capability from input and required output including recognition audio to text and synthesis text to audio",configuration_payload:"keep endpoint identity and payload separate and never hardcode credentials",result_cancellation:"inspect per item result error and cancellation and never fabricate content",simulation_authority:"offline output proves no live readiness and never authorizes disclosure or external action"};for(const[dimension,value]of Object.entries(textSpeechExplanation))await page.getByLabel(`Closed-note text speech ${dimension}`,{exact:true}).fill(value);await page.getByRole("button",{name:"Exit Text and Speech Patterns",exact:true}).click();await page.getByRole("button",{name:"Resume Text and Speech Patterns",exact:true}).click();if(await page.getByLabel("Closed-note text speech result_cancellation",{exact:true}).inputValue()!==textSpeechExplanation.result_cancellation)throw new Error("Text/Speech explanation reset");const textSpeechExplanationDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(textSpeechExplanationDraft.includes("never fabricate content")||textSpeechExplanationDraft.includes("never hardcode credentials"))throw new Error("Text/Speech explanation persisted");await page.getByRole("button",{name:"Check text/speech explanation",exact:true}).click();await page.getByText("4/4 · PASS",{exact:true}).waitFor();await page.getByRole("checkbox",{name:/produced this text\/speech explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const textSpeechContinue=page.getByRole("button",{name:"Continue",exact:true});await textSpeechContinue.waitFor();if(!await textSpeechContinue.evaluate(element=>element===document.activeElement))throw new Error("Text/Speech mastery did not focus Continue");const textSpeechMastery=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).textSpeechPatternEvidence,{key:saveKey});if(textSpeechMastery?.masteryStatus!=="mastered"||textSpeechMastery?.attemptCount!==16)throw new Error(`Text/Speech mastery incomplete ${JSON.stringify(textSpeechMastery)}`);if(["inputText","audioBytes","audioPath","transcript","generatedAudio","endpoint","credential","serviceResponse","externalActionRequest","response","choices","freeText"].some(key=>key in textSpeechMastery))throw new Error("Text/Speech private data persisted");await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const restoredTextSpeechContinue=page.getByRole("button",{name:"Continue",exact:true});await restoredTextSpeechContinue.waitFor();if(!await restoredTextSpeechContinue.evaluate(element=>element===document.activeElement))throw new Error("Text/Speech reload did not focus Continue");await restoredTextSpeechContinue.click();
