...
print("Operator:", learner)`);
  ...
  await floodedChoir.waitFor();
  if (await floodedChoir.getAttribute("data-flooded-choir-state") !== "complete") throw new Error("Host 09 did not expose read-only completion after both owned lessons");
  await floodedChoir.click();
  if (await page.locator('[data-terminal-exercise="EX-L0401-TEXT-ANALYSIS"], [data-terminal-exercise="EX-L0402-SPEECH-WORKLOADS"]').count()) throw new Error("Completed Host 09 reopened an owned lesson");
  await page.reload();
  await page.getByRole("button", { name: "Resume signal" }).click();
