...
print("Operator:", learner)`);
  ...
  await page.getByRole("button",{name:"Start Client Boundaries",exact:true}).click();await page.locator('[data-terminal-exercise="EX-L0503-CLIENT-BOUNDARIES"]').waitFor();await page.getByRole("button",{name:"Run deterministic mock",exact:true}).click();await page.getByText("P01",{exact:false}).waitFor();
  await page.getByLabel("Client boundary decision",{exact:true}).selectOption("named_model_deployment");await page.getByLabel("Client boundary reason",{exact:true}).selectOption("endpoint_selects_identity_and_model");await page.getByRole("button",{name:"Check client boundary",exact:true}).click();await page.getByRole("status").getByText("0/2",{exact:false}).waitFor();for(const d of["decision","reason"]){const f=page.getByLabel(`Client boundary ${d}`,{exact:true});if(await f.getAttribute("aria-invalid")!=="true"||await f.getAttribute("aria-describedby")!==`client-boundary-${d}-feedback`)throw new Error(`Client boundary ${d} remediation missing`);}await page.setViewportSize({width:640,height:480});await page.screenshot({path:qaPath("client-boundaries-primary-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});
  for(const id of Object.keys(referenceBoundaryPrimary)){const a=referenceBoundaryPrimary[id];await page.getByLabel("Client boundary decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Client boundary reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check client boundary",exact:true}).click();await page.getByText("Choice confirmed",{exact:false}).waitFor();await page.getByRole("button",{name:id==="P06"?"View primary result":"Next scenario",exact:true}).click();}await page.getByRole("radio",{name:"medium",exact:true}).check();await page.getByRole("button",{name:"Acknowledge primary form",exact:true}).click();
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();await page.getByRole("button",{name:"Start Client Boundary Transfer",exact:true}).click();await page.getByLabel("Client boundary decision",{exact:true}).selectOption("named_model_deployment");await page.getByLabel("Client boundary reason",{exact:true}).selectOption("endpoint_selects_identity_and_model");await page.getByRole("button",{name:"Check client boundary",exact:true}).click();await page.setViewportSize({width:320,height:240});await page.screenshot({path:qaPath("client-boundaries-transfer-remediation-qa.png"),fullPage:true});await page.setViewportSize({width:1600,height:900});await page.getByRole("button",{name:"Exit Client Boundaries",exact:true}).click();await page.getByRole("button",{name:"Resume Client Boundaries",exact:true}).click();if(await page.getByLabel("Client boundary reason",{exact:true}).inputValue()!=="endpoint_selects_identity_and_model")throw new Error("Client boundary transfer reset");const cbd=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(cbd.includes("endpoint_selects_identity_and_model")||cbd.includes("authorization error"))throw new Error("Client boundary content persisted");
  for(const id of Object.keys(referenceBoundaryTransfer)){const a=referenceBoundaryTransfer[id];await page.getByLabel("Client boundary decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Client boundary reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check client boundary",exact:true}).click();await page.getByText("Choice confirmed",{exact:false}).waitFor();await page.getByRole("button",{name:id==="T06"?"Begin closed-note explanation":"Next scenario",exact:true}).click();}for(const d of["configuration","client_layers","request_response","simulation_authority"])await page.getByLabel(`Closed-note client boundary ${d}`,{exact:true}).fill("wrong");await page.getByRole("button",{name:"Check client explanation",exact:true}).click();await page.getByRole("status").getByText("0/4",{exact:false}).waitFor();await page.screenshot({path:qaPath("client-boundaries-closed-note-qa.png"),fullPage:true});assertDistinctCaptures(["client-boundaries-primary-qa.png","client-boundaries-transfer-remediation-qa.png","client-boundaries-closed-note-qa.png"]);
  const cbe={configuration:"endpoint address credential identity deployment selects model",client_layers:"project client configuration then compatible inference client",request_response:"send model and input then read returned output",simulation_authority:"mock proves only local flow never authorizes live or destructive action"};for(const[d,v]of Object.entries(cbe))await page.getByLabel(`Closed-note client boundary ${d}`,{exact:true}).fill(v);await page.getByRole("button",{name:"Check client explanation",exact:true}).click();await page.getByText("Complete client-boundary explanation confirmed",{exact:false}).waitFor();await page.getByRole("checkbox",{name:/produced this client-boundary explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const cbm=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).clientBoundaryEvidence,{key:saveKey});if(cbm?.masteryStatus!=="mastered"||cbm?.mockPassed!==true)throw new Error(`Client boundary mastery incomplete ${JSON.stringify(cbm)}`);if(["endpoint","deploymentName","credential","requestInput","responseOutput","learnerSource","externalActionRequest","freeText"].some(k=>k in cbm))throw new Error("Client boundary private data persisted");
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredPromptContinue = page.getByRole("button", { name: "Continue", exact: true }); await restoredPromptContinue.waitFor(); await restoredPromptContinue.click();
  ...
  console.log(JSON.stringify({
    ...
    persistence: true,
    runtimeErrors: false,
    questions: ["HA-PY-001", "HA-PY-002", "HA-PY-003", "HA-AI901-001", "HA-AI901-RAI-MASTERY", "HA-AI901-MODEL-MASTERY", "HA-PY-STRUCTURED-PACKETS", "HA-PY-CONTROL-FLOW", "HA-PY-CLIENT-BRIDGE", "HA-AI901-TEXT-ANALYSIS", "HA-AI901-SPEECH-WORKLOADS", "HA-AI901-VISUAL-WORKLOADS", "HA-AI901-EXTRACTION-WORKLOADS", "HA-AI901-PORTAL-ORIENTATION", "HA-AI901-PROMPT-LAYERS"],
    credits: true,
  }));
} finally {
  await browser.close();
