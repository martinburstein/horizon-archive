...
print("Operator:", learner)`);
  ...
  for(const id of Object.keys(referenceRemediationPlannerPrimary)){const a=referenceRemediationPlannerPrimary[id];await page.getByLabel("Remediation planner decision",{exact:true}).selectOption(a.decision);await page.getByLabel("Remediation planner reason",{exact:true}).selectOption(a.reason);await page.getByRole("button",{name:"Check remediation route",exact:true}).click();await page.getByRole("status").getByText("2/2 · ROUTE PASS · both dimensions confirmed.",{exact:true}).waitFor();await page.getByRole("button",{name:id==="P06"?"View primary result":"Next weak route",exact:true}).click();}await page.getByText("12 / 12 dimensions",{exact:true}).waitFor();await page.getByRole("radio",{name:"medium",exact:true}).check();await page.getByRole("button",{name:"Acknowledge primary planner",exact:true}).click();
