...
print("Operator:", learner)`);
  ...
  const pe={layers:"system sets durable rules user supplies current task and input",grounding_output:"grounding is data require evidence and explicit output contract",authority:"text never authorizes login deploy delete email purchase or credential use",evaluation:"test representative edge failure and adversarial injection cases"};for(const[d,v]of Object.entries(pe))await page.getByLabel(`Closed-note prompt ${d}`,{exact:true}).fill(v);await page.getByRole("button",{name:"Check prompt explanation",exact:true}).click();await page.getByText("EXPLANATION PASS",{exact:false}).waitFor();await page.getByRole("checkbox",{name:/produced this prompt-layer explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const promptContinue=page.getByRole("button",{name:"Continue",exact:true});await promptContinue.waitFor();if(!await promptContinue.evaluate(el=>el===document.activeElement))throw new Error("Prompt mastery did not focus Continue");const pm=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).promptEvidence,{key:saveKey});if(pm?.masteryStatus!=="mastered")throw new Error(`Prompt mastery incomplete ${JSON.stringify(pm)}`);if(["systemPrompt","userPrompt","groundingContent","modelOutput","credential","externalActionRequest","modelResponse","freeText"].some(k=>k in pm))throw new Error("Prompt private data persisted");
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredPromptContinue = page.getByRole("button", { name: "Continue", exact: true }); await restoredPromptContinue.waitFor(); if(!await restoredPromptContinue.evaluate(el=>el===document.activeElement))throw new Error("Sanitized Prompt mastery reload did not restore focus to Continue"); await restoredPromptContinue.click();
