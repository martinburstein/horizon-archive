...
print("Operator:", learner)`);
  ...
  await assertSingleAgentContinuity(page,"closed-note");for(const dimension of ["fit_instructions","least_privilege","failure_safety","client_flow"])await page.getByLabel(`Closed-note single agent ${dimension}`,{exact:true}).fill("wrong");await page.getByRole("button",{name:"Check single-agent explanation",exact:true}).click();await page.getByRole("status").getByText("0/4 · REMEDIATE",{exact:true}).waitFor();for(const dimension of ["fit_instructions","least_privilege","failure_safety","client_flow"]){const field=page.getByLabel(`Closed-note single agent ${dimension}`,{exact:true});if(await field.getAttribute("aria-invalid")!=="true"||await field.getAttribute("aria-describedby")!==`single-agent-explanation-${dimension}-feedback`)throw new Error(`Single Agent closed-note ${dimension} remediation missing`);}await page.screenshot({path:qaPath("single-agent-closed-note-qa.png"),fullPage:true});assertDistinctCaptures(["single-agent-primary-qa.png","single-agent-transfer-remediation-qa.png","single-agent-closed-note-qa.png"]);
  const singleAgentExplanation={fit_instructions:"use agent only for goal tools orchestration with stable instructions",least_privilege:"attach only required tools and tool capability is not permission",failure_safety:"test edge injection failure and denied paths never fabricate success",client_flow:"use agent identifier submit input then read result or error"};for(const[dimension,value]of Object.entries(singleAgentExplanation))await page.getByLabel(`Closed-note single agent ${dimension}`,{exact:true}).fill(value);await page.getByRole("button",{name:"Exit Single Agent",exact:true}).click();await page.getByRole("button",{name:"Resume Single Agent",exact:true}).click();if(await page.getByLabel("Closed-note single agent failure_safety",{exact:true}).inputValue()!==singleAgentExplanation.failure_safety)throw new Error("Single Agent explanation reset after close/reopen");const singleAgentExplanationDraft=await page.evaluate(({key})=>localStorage.getItem(key),{key:saveKey});if(singleAgentExplanationDraft.includes("never fabricate success")||singleAgentExplanationDraft.includes("tool capability is not permission"))throw new Error("Single Agent explanation persisted");await page.getByRole("button",{name:"Check single-agent explanation",exact:true}).click();await page.getByRole("status").getByText("4/4 · PASS",{exact:true}).waitFor();await page.getByRole("checkbox",{name:/produced this single-agent explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const singleAgentContinue=page.getByRole("button",{name:"Continue",exact:true});await singleAgentContinue.waitFor();if(!await singleAgentContinue.evaluate(element=>element===document.activeElement))throw new Error("Single Agent mastery did not focus Continue");const singleAgentMastery=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).singleAgentEvidence,{key:saveKey});if(singleAgentMastery?.masteryStatus!=="mastered"||singleAgentMastery?.attemptCount!==16)throw new Error(`Single Agent mastery incomplete ${JSON.stringify(singleAgentMastery)}`);if(["agentInstructions","toolPayload","toolResult","agentId","endpoint","credential","conversationText","externalActionRequest","freeText","response","choices"].some(key=>key in singleAgentMastery))throw new Error("Single Agent private data persisted");
  await page.reload();await page.getByRole("button",{name:"Resume signal"}).click();const restoredSingleAgentContinue=page.getByRole("button",{name:"Continue",exact:true});await restoredSingleAgentContinue.waitFor();if(!await restoredSingleAgentContinue.evaluate(element=>element===document.activeElement))throw new Error("Sanitized Single Agent mastery reload did not restore focus to Continue");await restoredSingleAgentContinue.click();
  ...
  console.log(JSON.stringify({
    ...
    persistence: true,
    runtimeErrors: false,
    questions: ["HA-PY-001", "HA-PY-002", "HA-PY-003", "HA-AI901-001", "HA-AI901-RAI-MASTERY", "HA-AI901-MODEL-MASTERY", "HA-PY-STRUCTURED-PACKETS", "HA-PY-CONTROL-FLOW", "HA-PY-CLIENT-BRIDGE", "HA-AI901-TEXT-ANALYSIS", "HA-AI901-SPEECH-WORKLOADS", "HA-AI901-VISUAL-WORKLOADS", "HA-AI901-EXTRACTION-WORKLOADS", "HA-AI901-PORTAL-ORIENTATION", "HA-AI901-PROMPT-LAYERS", "HA-AI901-CLIENT-BOUNDARIES"],
    credits: true,
  }));
} finally {
  await browser.close();
...
async function assertSingleAgentContinuity(page, phase) {
  ...
  if (await dialog.getAttribute("aria-describedby") !== "single-agent-offline-warning") throw new Error(`Single Agent ${phase} warning association missing`);
  await page.locator("#single-agent-offline-warning").getByText("no agent, tool, service, Azure, or external action", { exact: false }).waitFor();
