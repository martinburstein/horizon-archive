...
print("Operator:", learner)`);
  ...
  const cbe={configuration:"endpoint address credential identity deployment selects model",client_layers:"project client configuration then compatible inference client",request_response:"send model and input then read returned output",simulation_authority:"mock proves only local flow never authorizes live or destructive action"};for(const[d,v]of Object.entries(cbe))await page.getByLabel(`Closed-note client boundary ${d}`,{exact:true}).fill(v);await page.getByRole("button",{name:"Check client explanation",exact:true}).click();await page.getByText("EXPLANATION PASS",{exact:false}).waitFor();await page.getByRole("checkbox",{name:/produced this client-boundary explanation/i}).check();await page.getByRole("radio",{name:"high",exact:true}).check();await page.getByRole("button",{name:"Acknowledge strict mastery",exact:true}).click();const clientBoundaryContinue=page.getByRole("button",{name:"Continue",exact:true});await clientBoundaryContinue.waitFor();if(!await clientBoundaryContinue.evaluate(el=>el===document.activeElement))throw new Error("Client Boundaries mastery did not focus Continue");const cbm=await page.evaluate(({key})=>JSON.parse(localStorage.getItem(key)).clientBoundaryEvidence,{key:saveKey});if(cbm?.masteryStatus!=="mastered"||cbm?.mockPassed!==true)throw new Error(`Client boundary mastery incomplete ${JSON.stringify(cbm)}`);if(["endpoint","deploymentName","credential","requestInput","responseOutput","learnerSource","externalActionRequest","freeText"].some(k=>k in cbm))throw new Error("Client boundary private data persisted");
  await page.reload(); await page.getByRole("button", { name: "Resume signal" }).click(); const restoredClientBoundaryContinue = page.getByRole("button", { name: "Continue", exact: true }); await restoredClientBoundaryContinue.waitFor(); if(!await restoredClientBoundaryContinue.evaluate(el=>el===document.activeElement))throw new Error("Sanitized Client Boundaries mastery reload did not restore focus to Continue"); await restoredClientBoundaryContinue.click();
