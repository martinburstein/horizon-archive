...
def validate_bank(b):
 req=set(b["workloads"]);assert len(req)==3
 for form,items in b["forms"].items():
  assert len(items)==6 and len({x["id"] for x in items})==6,form
  assert {x["topic"] for x in items if x["topic"]!="client_flow"}==req
  assert sum(x["topic"]=="client_flow" for x in items)==3
def evaluate(form,a):
...
def self_test():
 ...
 w=load("reference_primary_answers.json");w["P01"]["decision"]="speech_synthesis";assert not evaluate("primary",w)["P01.decision"]
 w=load("reference_primary_answers.json");w["P06"]["reason"]="assume_success";assert not evaluate("primary",w)["P06.reason"]
...
def main():
 ...
 validate_bank(load("scenario_bank.json"));r=evaluate(a.form,json.loads(Path(a.check).read_text()));s=sum(r.values());print(f"{a.form.upper()}: {s}/12")
