...
test("first-contact semantics and focus geometry remain unchanged", async () => {
  ...
  assert.match(app, /ref=\{hotspot\.primary \? primaryHotspotRef : isFractureNursery \? fractureNurseryRef : undefined\}/);
...
test("primary Terminal focus is localized while every other hotspot keeps the shared cue", async () => {
  ...
  assert.match(styles, /\.hotspot:is\(\[data-hotspot-id="route-marker"\], \[data-hotspot-id="fracture-nursery"\]\) span \{[\s\S]*?opacity: 1;/);
