...
try {
  for(const [id,viewport] of Object.entries(FIRST_RUN_RESPONSIVE_LAYOUTS)){
    ...
    const width=`max(${semanticTarget.width/38.4}%, 44px)`;const height=`max(${semanticTarget.height/21.6}%, 44px)`;const centerX=(semanticTarget.x+semanticTarget.width/2)/38.4;const centerY=(semanticTarget.y+semanticTarget.height/2)/21.6;const hotspot={left:`clamp(0px, calc(${centerX}% - ${width} / 2), calc(100% - ${width}))`,top:`clamp(0px, calc(${centerY}% - ${height} / 2), calc(100% - ${height}))`,width,height};
