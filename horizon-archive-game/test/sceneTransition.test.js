...
test("App moves focus and announces the scene while preserving the strict route gate", () => {
  ...
  assert.match(appSource, /sceneArrivalFocusPendingRef\.current = true;[\s\S]*?setSceneAnnouncement\(buildSceneArrivalAnnouncement\(nextScene\)\);[\s\S]*?setDialogue\(nextScene\.prompt, "system"\);/);
