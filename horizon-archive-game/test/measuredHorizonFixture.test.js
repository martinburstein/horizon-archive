...
test("TD012 all 58 product surfaces render exact visible owner and a real declared focus target", { timeout: 60_000 }, async () => {
  ...
  try { const { MeasuredHorizon } = await vite.ssrLoadModule("/src/MeasuredHorizon.jsx"); for (const id of measuredHorizonScenarioNames) { const scenario = createMeasuredHorizonScenario(id); const markup = renderToStaticMarkup(React.createElement(MeasuredHorizon,{state:scenario.state,onAction(){},onFieldChange(){}})); const renderedIds=[...markup.matchAll(/\sid="([^"]+)"/g)].map((match)=>match[1]); assert.ok(markup.includes(`data-active-owner="${scenario.declaredOwner}"`),`${id} owner`); assert.ok(markup.includes(`>${scenario.declaredOwner}</p>`),`${id} visible owner`); assert.ok(markup.includes(`id="${scenario.declaredFocus}"`),`${id} focus`); assert.equal(new Set(renderedIds).size,renderedIds.length,`${id} unique ids`); assert.equal((markup.match(/<main\b/g)??[]).length,1); assert.equal((markup.match(/role="status"/g)??[]).length,1); } }
  finally { await vite.close(); }
});

test("TD012 browser harness compares visible owner and actual active element", () => {
  const source=readFileSync(new URL("../review-fixtures/td012-measured-horizon/ReviewMeasuredHorizonFixture.jsx",import.meta.url),"utf8"); const css=readFileSync(new URL("../src/styles.css",import.meta.url),"utf8"); assert.match(source,/document\.activeElement\?\.id/); assert.match(source,/renderedOwner===scenario\.declaredOwner&&activeElementId===scenario\.declaredFocus/); assert.match(source,/data-rendered-contract=/); assert.match(css,/\.measured-horizon-outcome h2\{min-height:3\.6em\}/);
});
