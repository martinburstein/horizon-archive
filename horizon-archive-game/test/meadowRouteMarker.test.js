...
test("persistent route label remains inset inside its unchanged hotspot", () => {
  ...
  assert.match(appSource, /<span>\{verb\} \{hotspot\.label\}\{routeMarkerLabel\}\{nurseryLabel\}\{sixfoldLabel\}<\/span>/);
  assert.match(styleSource, /\.hotspot:is\(\[data-hotspot-id="route-marker"\], \[data-hotspot-id="fracture-nursery"\], \[data-hotspot-id="sixfold-weir"\]\) span \{[\s\S]*?left: 3px;[\s\S]*?right: 3px;[\s\S]*?transform: none;[\s\S]*?width: auto;[\s\S]*?max-width: none;[\s\S]*?white-space: normal;[\s\S]*?opacity: 1;/);
});
