...
test("TD011 production and closed fixture serve exact fresh-build bytes", { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    try { assert.equal(await verifyServedTree({ server: production, localRoot: join(gameRoot, "dist"), baseUrl: "http://127.0.0.1:4290" }), 23); } finally { await closePreview(production); }
