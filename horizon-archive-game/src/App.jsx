...
import { evaluateMixedSimulationItem,getMixedSimulationFeedback,getMixedSimulationOptions,mixedSimulationDialogDescribedBy,mixedSimulationDimensions,mixedSimulationExercise,mixedSimulationItems,sanitizeMixedSimulationEvidence,updateMixedSimulationEvidence } from "./mixedSimulationExercise.js";
...
export function App() {
  ...
  function openMixedSimulation(){setTerminalOpen(true);setRuinsTerminalKind("mixed-simulation");if(!mixedSimulationSession)setMixedSimulationSession({index:0,response:{decision:"",reason:""},result:null,hintLevel:0,complete:false,timingEnabled:false,elapsedSeconds:mixedSimulationEvidence?.optionalElapsedSeconds||0});}
  function exitMixedSimulation(){if(mixedSimulationSession)setMixedSimulationEvidence(p=>updateMixedSimulationEvidence(p,{optionalElapsedSeconds:mixedSimulationSession.elapsedSeconds}));setTerminalOpen(false);setRuinsTerminalKind(null);setDialogue("Mixed simulation closed. Completed item evidence remains; working choices are session-only. No service or external action occurred.","system");}
      ...
      <section className="scene-frame" aria-label={`${scene.location} scene`}>
        ...
        {terminalOpen&&scene.id==="ruins"&&ruinsTerminalKind==="mixed-simulation"&&mixedSimulationSession&&(
          <TerminalShell exerciseId={mixedSimulationExercise.exercise_id} title="Offline AI-901 Mixed Simulation" filename="sim_01_mixed.json" lessonId="SIM-01" statusText={mixedSimulationSession.complete?"24/24 COMPLETE":`ITEM ${mixedSimulationSession.index+1}/12`} closeLabel="Exit Mixed Simulation" describedBy={mixedSimulationDialogDescribedBy} restoreFocusTo={terminalTriggerRef.current} onClose={exitMixedSimulation}>
            <section className="model-choice-workspace mixed-simulation-workspace"><p id="mixed-simulation-offline-warning" className="model-choice-boundary" role="note">COURSE-AUTHORED OFFLINE PRACTICE · not Microsoft exam content and no exam guarantee. No service, credential, Azure change, communication, disclosure, publication, purchase, deletion, or external action occurs or is authorized. No exam text, notes, content, credential, endpoint, payload, response, action request, choice, or free text persists.</p><p id="mixed-simulation-text-equivalent" className="speech-transcript">Mixed text equivalent: twelve original scenarios, five concept/capability and seven implementation items, collectively cover all fifteen objectives. Every item requires decision plus reason. Mastery is exactly 24/24; timing is optional diagnostic data and untimed mode is fully equivalent.</p><label className="ownership-confirmation"><input type="checkbox" checked={mixedSimulationSession.timingEnabled} onChange={e=>setMixedSimulationSession({...mixedSimulationSession,timingEnabled:e.target.checked})}/>Enable optional 25-minute diagnostic timer</label><p role="timer">{mixedSimulationSession.timingEnabled?`Diagnostic elapsed: ${mixedSimulationSession.elapsedSeconds} seconds. No time limit; timing cannot change mastery.`:"Untimed mode active · fully equivalent for mastery."}</p>{mixedSimulationSession.complete?<section className="workload-summary"><p className="pane-label">901 TEACHER // STRICT BLOCK COMPLETE</p><h2>24 / 24 dimensions</h2><p>All 12 items and all 15 objective tags pass. This supports continued practice only.</p><fieldset className="confidence-group"><legend>Confidence (not mastery)</legend>{["low","medium","high"].map(v=><label key={v}><input type="radio" name="mixed-simulation-confidence" checked={mixedSimulationEvidence?.confidence===v} onChange={()=>setMixedSimulationEvidence(p=>updateMixedSimulationEvidence(p,{confidence:v}))}/>{v}</label>)}</fieldset><button className="confirm-action" type="button" onClick={acknowledgeMixedSimulationMastery}>Acknowledge SIM-01 mastery</button></section>:(()=>{const item=mixedSimulationItems[mixedSimulationSession.index],opts=getMixedSimulationOptions(item.id),f=getMixedSimulationFeedback(item,mixedSimulationSession.result);return <form className="model-choice-form" onSubmit={checkMixedSimulation}><header><p className="pane-label">PILOT // MIXED SIMULATION OWNER · {item.id} · {item.domain}</p><h2>{item.prompt}</h2><p>Objective tags: {item.objective_ids.join(" · ")}</p></header><div className="model-choice-fields">{mixedSimulationDimensions.map(d=>{const ok=mixedSimulationSession.result?.correctness[d],id=`mixed-simulation-${d}-feedback`;return <label key={d}><span>{d}</span><select aria-label={`Mixed simulation ${d}`} aria-invalid={mixedSimulationSession.result?!ok:undefined} aria-describedby={mixedSimulationSession.result?id:undefined} value={mixedSimulationSession.response[d]} onChange={e=>setMixedSimulationSession({...mixedSimulationSession,response:{...mixedSimulationSession.response,[d]:e.target.value},result:null})}><option value="">Choose one</option>{opts[d].map(v=><option key={v} value={v}>{formatChoice(v)}</option>)}</select>{mixedSimulationSession.result&&<small id={id}>{ok?"SYSTEM // Correct.":"901 TEACHER // Repair this objective boundary."}</small>}</label>;})}</div><section className="terminal-console model-choice-output"><div className="console-heading-row"><strong>SYSTEM // STRICT 24-DIMENSION VALIDATOR</strong><button className="run-action" type="submit">Check simulation item</button></div><div role="status" aria-live="polite">{f.systemScore}</div>{f.teacherRemediation&&<p className="teacher-remediation"><strong>901 TEACHER // OBJECTIVE REMEDIATION</strong><span>{f.teacherRemediation}</span></p>}{mixedSimulationSession.result?.passed&&<button className="confirm-action" type="button" onClick={advanceMixedSimulation}>{mixedSimulationSession.index===11?"View strict result":"Next mixed item"}</button>}</section></form>;})()}</section>
          </TerminalShell>)}
            ...
            <div className="dialogue-copy">
              ...
              <div className="dialogue-footer">
                ...
                <div className="dialogue-actions">
                  ...
                  {pendingAdvance&&scene.id==="ruins"&&capstoneReadinessEvidence?.masteryStatus==="mastered"&&mixedSimulationEvidence?.masteryStatus!=="mastered"&&<button ref={continueButtonRef} className="continue-action" data-terminal-focus-fallback onClick={e=>{terminalTriggerRef.current=e.currentTarget;openMixedSimulation();}}>{mixedSimulationSession?"Resume Mixed Simulation":"Continue"}</button>}
