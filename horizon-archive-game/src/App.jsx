...
export function App() {
  ...
  function openRemediationPlanner(){setTerminalOpen(true);setRuinsTerminalKind("remediation-planner");if(!remediationPlannerSession){const form=remediationPlannerEvidence?.masteryStatus==="primary_complete"?"transfer":remediationPlannerEvidence?.masteryStatus==="transfer_complete"?"explanation":"primary";setRemediationPlannerSession({form,phase:form==="explanation"?"explanation":"scenarios",index:0,response:{decision:"",reason:""},result:null,hintLevel:0,complete:false,explanationResponse:{gap_priority:"",source_route:"",practice_reassessment:"",stop_safeguards:""},explanationResult:null,ownershipConfirmed:false});}}
        ...
        {terminalOpen&&scene.id==="ruins"&&ruinsTerminalKind==="remediation-planner"&&remediationPlannerSession&&(
          <TerminalShell exerciseId={remediationPlannerExercise.exercise_id} title="Offline Remediation Planner" filename={remediationPlannerSession.phase==="explanation"?"closed_note.md":`${remediationPlannerSession.form}_routes.json`} lessonId={remediationPlannerExercise.lesson_id} statusText={remediationPlannerSession.phase==="explanation"?"CLOSED-NOTE GATE":remediationPlannerSession.form.toUpperCase()} closeLabel="Exit Remediation Planner" describedBy={remediationPlannerDialogDescribedBy} restoreFocusTo={terminalTriggerRef.current} onClose={exitRemediationPlanner}>
            <section className="model-choice-workspace remediation-planner-workspace">
              ...
              <aside className="task-pane remediation-route-queue" aria-label="Weak objective route queue"><p className="pane-label">SYSTEM // SANITIZED ROUTE STATE</p><p>{Object.keys(remediationPlannerEvidence?.routes||{}).length} / 12 course-authored routes recorded</p><ul>{Object.entries(remediationPlannerEvidence?.routes||{}).map(([id,route])=><li key={id}><strong>{id} · {route.objectiveId}</strong> · {route.failedDimension.replaceAll("_"," ")} · {route.mappedLessonId} · {route.officialSourceId} · {route.reassessmentStatus}</li>)}</ul></aside>
            ...
            <div className="dialogue-copy">
              ...
              <div className="dialogue-footer">
                ...
                <div className="dialogue-actions">
                  ...
                  {pendingAdvance && (scene.id !== "ruins" || remediationPlannerEvidence?.masteryStatus === "mastered") && (
