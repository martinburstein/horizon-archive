...
export function App() {
  ...
  const finalConfidenceActionRef = useRef(null);
  ...
  useLayoutEffect(() => {if(!focusContinueAfterMixedSimulationRef.current||terminalOpen||!pendingAdvance||mixedSimulationEvidence?.masteryStatus!=="mastered")return;focusContinueAfterMixedSimulationRef.current=false;(finalConfidenceActionRef.current??continueButtonRef.current)?.focus({preventScroll:true});},[terminalOpen,pendingAdvance,mixedSimulationEvidence?.masteryStatus]);
