...
const FINAL_CONFIDENCE_ENTRY_ERRORS = {
  ...
  cum01Transfer: "CUM-01 transfer must be 16 out of 16.",
  sim01: "SIM-01 must be 24 out of 24.",
  sim02: "SIM-02 must be 24 out of 24.",
  spacing: "The prior simulations must be separated by at least 48 hours.",
  criticalMisconceptions: "Resolve every open critical misconception before this form.",
  highConfidenceRetest: "Explain and retest every high-confidence miss on fresh practice.",
  sourceFreshness: "Use an attempt date no earlier than, and no more than seven days after, official-source verification.",
};

function finalConfidenceEntryA11y(gate, checkName) {
  if (gate?.checks?.[checkName] !== false) return {};
  return {
    "aria-invalid": "true",
    "aria-describedby": `final-confidence-entry-${checkName}-feedback`,
  };
}
...
export function App() {
  ...
  const [tidalPatternLoomDecodedImage, setTidalPatternLoomDecodedImage] = useState(null);
  const [tidalPatternLoomPresented, setTidalPatternLoomPresented] = useState(false);
  ...
  const custodyLedgerExplanationSubmissionControllerRef = useRef(null);
  ...
  const tidalPatternLoomRef = useRef(null);
  const tidalPatternLoomTransitionPendingRef = useRef(false);
  const tidalPatternLoomFocusPendingRef = useRef(false);
  const tidalPatternLoomAnnouncementPendingRef = useRef(false);
  const waterlineLedgerRef = useRef(null);
  const waterlineLedgerFocusPendingRef = useRef(false);
  const waterlineLedgerAnnouncementPendingRef = useRef(false);
  const waterlineLedgerActivationRef = useRef(false);
  const crownCounterpoiseRef = useRef(null);
  const crownCounterpoiseActivationRef = useRef(false);
  ...
  const finalConfidenceTimerRef = useRef(null);
  ...
  const legacyHost12LessonLauncherVisible = isLegacyHost12LessonLauncherVisible(DROWNED_SWITCHYARD_REGISTRY.source);
  const legacyHost13LessonLauncherVisible = isLegacyHost13LessonLauncherVisible(TIDAL_PATTERN_LOOM_REGISTRY.source);
  ...
  const tidalPatternLoomState=deriveTidalPatternLoomState({predecessorComplete:drownedSwitchyardState==="complete",textSpeechPatternEvidence,visualPatternEvidence,registry:TIDAL_PATTERN_LOOM_REGISTRY,provenance:TIDAL_PATTERN_LOOM_PROVENANCE,decodedImage:tidalPatternLoomDecodedImage});
  const tidalPatternLoomLawful=tidalPatternLoomState!=="hidden";
  const tidalPatternLoomActive=tidalPatternLoomPresented&&tidalPatternLoomLawful;
  const waterlineLedgerState=deriveWaterlineLedgerState({predecessorComplete:tidalPatternLoomState==="complete",objectiveLedgerEvidence,remediationPlannerEvidence,registry:WATERLINE_LEDGER_REGISTRY,decodedImage:waterlineLedgerDecodedImage});
  const waterlineLedgerLawful=waterlineLedgerState.state!=="hidden";
  ...
  const tidalPatternLoomHotspot=getTidalPatternLoomHotspot(TIDAL_PATTERN_LOOM_REGISTRY);
  const tidalPatternLoomHotspots=scene.id==="ruins"&&tidalPatternLoomActive&&tidalPatternLoomHotspot?[{id:"tidal-pattern-loom",label:TIDAL_PATTERN_LOOM_COPY.name,hotspot:tidalPatternLoomHotspot}]:[];
  const waterlineLedgerHotspot=getWaterlineLedgerHotspot(WATERLINE_LEDGER_REGISTRY);
  ...
  const presentedSceneHotspots = scene.id === "automaton" && witnessSpineNativeActive
    ...
    : scene.id === "ruins" && tidalPatternLoomActive
    ? tidalPatternLoomHotspots
    : scene.id === "ruins" && drownedSwitchyardActive
  ...
  const hotspotButtons = presentedSceneHotspots.map((hotspot) => {
    ...
    const isTidalPatternLoom=scene.id==="ruins"&&hotspot.id==="tidal-pattern-loom";
    const isWaterlineLedger=scene.id==="ruins"&&hotspot.id==="waterline-ledger";
    const isCrownCounterpoise=scene.id==="ruins"&&hotspot.id==="crown-counterpoise";
    ...
    if(isCrownCounterpoise)return <button key={hotspot.id} ref={crownCounterpoiseRef} type="button" className={`hotspot hotspot-secondary waterline-ledger-hotspot crown-counterpoise-hotspot waterline-ledger-state-${crownCounterpoiseState.state}`} data-hotspot-id={hotspot.id} data-crown-counterpoise-state={crownCounterpoiseState.state} style={hotspotStyle} onClick={(event)=>{if(terminalOpen||crownCounterpoiseActivationRef.current||event.detail>1)return;crownCounterpoiseActivationRef.current=true;terminalTriggerRef.current=event.currentTarget;useHotspot(hotspot.id);queueMicrotask(()=>{crownCounterpoiseActivationRef.current=false;});}} disabled={terminalOpen} aria-label={`${verb} ${CROWN_COUNTERPOISE_COPY.name}`} aria-describedby="crown-counterpoise-physical-description crown-counterpoise-state-description"><span>{verb} {CROWN_COUNTERPOISE_COPY.name} // {crownCounterpoiseState.state.toUpperCase()}</span></button>;
    if(isWaterlineLedger)return <button key={hotspot.id} ref={waterlineLedgerRef} type="button" className={`hotspot hotspot-secondary waterline-ledger-hotspot waterline-ledger-state-${waterlineLedgerState.state}`} data-hotspot-id={hotspot.id} data-waterline-ledger-state={waterlineLedgerState.state} style={hotspotStyle} onClick={(event)=>{if(terminalOpen||waterlineLedgerActivationRef.current||event.detail>1)return;waterlineLedgerActivationRef.current=true;terminalTriggerRef.current=event.currentTarget;useHotspot(hotspot.id);queueMicrotask(()=>{waterlineLedgerActivationRef.current=false;});}} disabled={terminalOpen} aria-label={`${verb} ${WATERLINE_LEDGER_COPY.name}`} aria-describedby="waterline-ledger-physical-description waterline-ledger-state-description"><span>{verb} {WATERLINE_LEDGER_COPY.name} // {waterlineLedgerState.state.toUpperCase()}</span></button>;
    if(isTidalPatternLoom)return <button key={hotspot.id} ref={tidalPatternLoomRef} className="hotspot hotspot-secondary" data-hotspot-id={hotspot.id} data-tidal-pattern-loom-state={tidalPatternLoomState} style={hotspotStyle} onClick={(event)=>{terminalTriggerRef.current=event.currentTarget;useHotspot(hotspot.id);}} disabled={terminalOpen} aria-label={`${verb.toLowerCase()} ${TIDAL_PATTERN_LOOM_COPY.name}, ${tidalPatternLoomState}`}><span>{verb} {TIDAL_PATTERN_LOOM_COPY.name} // {tidalPatternLoomState.toUpperCase()}</span></button>;
  ...
  useEffect(()=>{if(TIDAL_PATTERN_LOOM_REGISTRY.source.enabled!==true||!TIDAL_PATTERN_LOOM_SOURCE_URL){setTidalPatternLoomDecodedImage(null);return undefined;}let connected=true;const image=new Image();image.decoding="async";image.onload=()=>connected&&setTidalPatternLoomDecodedImage({complete:image.complete,naturalWidth:image.naturalWidth,naturalHeight:image.naturalHeight});image.onerror=()=>connected&&setTidalPatternLoomDecodedImage(null);image.src=tidalPatternLoomImage;return()=>{connected=false;image.onload=null;image.onerror=null;};},[]);
  useEffect(()=>{if(!tidalPatternLoomLawful){tidalPatternLoomTransitionPendingRef.current=false;tidalPatternLoomAnnouncementPendingRef.current=false;if(tidalPatternLoomPresented)setTidalPatternLoomPresented(false);return undefined;}if(tidalPatternLoomPresented||mode!=="playing"||scene.id!=="ruins"||terminalOpen)return undefined;if(!tidalPatternLoomTransitionPendingRef.current){tidalPatternLoomFocusPendingRef.current=tidalPatternLoomState!=="complete";setTidalPatternLoomPresented(true);return undefined;}const frame=window.requestAnimationFrame(()=>{tidalPatternLoomTransitionPendingRef.current=false;tidalPatternLoomFocusPendingRef.current=true;tidalPatternLoomAnnouncementPendingRef.current=true;setTidalPatternLoomPresented(true);});return()=>window.cancelAnimationFrame(frame);},[mode,scene.id,terminalOpen,tidalPatternLoomLawful,tidalPatternLoomPresented,tidalPatternLoomState]);
  useLayoutEffect(()=>{if(!tidalPatternLoomFocusPendingRef.current||!tidalPatternLoomActive||terminalOpen)return;const target=tidalPatternLoomRef.current;if(!target?.isConnected)return;tidalPatternLoomFocusPendingRef.current=false;target.focus({preventScroll:true});if(tidalPatternLoomAnnouncementPendingRef.current){tidalPatternLoomAnnouncementPendingRef.current=false;setSceneAnnouncement(TIDAL_PATTERN_LOOM_COPY.available);}},[tidalPatternLoomActive,tidalPatternLoomState,terminalOpen]);
  ...
  useEffect(() => {
    ...
    if (!["RG-30", "save_confirmation", "recoverable_save_failure", "RG-U"].includes(
      custodyLedgerPrimaryView?.phase,
    )) {
      custodyLedgerRAIPrepareSaveConfirmationControllerRef.current = null;
    }
    if (!["RG-30", "save_confirmation", "recoverable_save_failure", "comparison_complete"].includes(
      custodyLedgerPrimaryView?.phase,
    )) {
  ...
  useLayoutEffect(() => {if(!focusContinueAfterObjectiveLedgerRef.current||terminalOpen||!pendingAdvance||objectiveLedgerEvidence?.masteryStatus!=="mastered")return;focusContinueAfterObjectiveLedgerRef.current=false;const target=waterlineLedgerNativeActive?waterlineLedgerRef.current:continueButtonRef.current;if(target?.isConnected)target.focus({preventScroll:true});if(waterlineLedgerNativeActive)setSceneAnnouncement(WATERLINE_LEDGER_COPY.remediation_in_progress);},[terminalOpen,pendingAdvance,objectiveLedgerEvidence?.masteryStatus,waterlineLedgerNativeActive]);

  useLayoutEffect(() => {if(!focusContinueAfterRemediationPlannerRef.current||terminalOpen||!pendingAdvance||remediationPlannerEvidence?.masteryStatus!=="mastered")return;focusContinueAfterRemediationPlannerRef.current=false;const target=crownCounterpoiseNativeActive?crownCounterpoiseRef.current:waterlineLedgerNativeActive?waterlineLedgerRef.current:continueButtonRef.current;if(target?.isConnected)target.focus({preventScroll:true});setSceneAnnouncement(crownCounterpoiseNativeActive?CROWN_COUNTERPOISE_COPY.available:WATERLINE_LEDGER_COPY.mastered);},[terminalOpen,pendingAdvance,remediationPlannerEvidence?.masteryStatus,crownCounterpoiseNativeActive,waterlineLedgerNativeActive]);
  useLayoutEffect(() => {if(!focusContinueAfterCapstoneReadinessRef.current||terminalOpen||!pendingAdvance||capstoneReadinessEvidence?.masteryStatus!=="mastered")return;focusContinueAfterCapstoneReadinessRef.current=false;const target=crownCounterpoiseNativeActive?crownCounterpoiseRef.current:continueButtonRef.current;target?.focus({preventScroll:true});},[terminalOpen,pendingAdvance,capstoneReadinessEvidence?.masteryStatus,crownCounterpoiseNativeActive]);
  useLayoutEffect(() => {if(!focusContinueAfterMixedSimulationRef.current||terminalOpen||!pendingAdvance||mixedSimulationEvidence?.masteryStatus!=="mastered")return;focusContinueAfterMixedSimulationRef.current=false;const target=crownCounterpoiseNativeActive?crownCounterpoiseRef.current:continueButtonRef.current;target?.focus({preventScroll:true});},[terminalOpen,pendingAdvance,mixedSimulationEvidence?.masteryStatus,crownCounterpoiseNativeActive]);
  ...
  function followRecordedCivicRoute(event) {
    ...
    custodyLedgerExplanationSubmissionControllerRef.current = null;
    custodyLedgerRAIPrimaryEntryControllerRef.current = null;
    custodyLedgerRAIPrimaryConvergenceControllerRef.current = null;
    custodyLedgerRAITransferConvergenceControllerRef.current = null;
    setMode("rp002-arrival");
  }
  ...
  function clearCustodyLedgerPrimaryResult(routeState, event) {
    if (demoTour || typeof window === "undefined") return;
    const dismissalController = custodyLedgerPrimaryDismissalControllerRef.current;
    const primaryResult = dismissalController?.getState();
    const result = dismissalController?.dispatch({
      packetId: "RP-002",
    ...
    if (result?.status !== "fresh_practice_opened") return;
    const predecessor = readVerifiedCityThresholdPredecessor(window.localStorage);
    const transferController = predecessor
      ? createCustodyLedgerNormalTransferInteraction(routeState, primaryResult, result.state, predecessor)
      : null;
  ...
  function submitCustodyLedgerTransfer(routeState, fields, event) {
    ...
    const result = custodyLedgerTransferControllerRef.current?.dispatch({
      ...
      action: CUSTODY_LEDGER_SUBMIT_EXPEDITION_FIELDS,
      activationKind: routeActivationKind(event),
      eventToken: routeEventToken("rp002-transfer-submit"),
      classification: fields.classification,
      fieldOwner: fields.owner,
    });
    ...
    if (result.status === "transfer_evidence_complete") {
      ...
      custodyLedgerTransferContextRef.current = {
        ...context,
        transferCompleteState: result.state,
      };
    }
    setCustodyLedgerPrimaryView(result.state);
  }
  ...
  function openCustodyLedgerExplanation(event) {
    ...
    if (result?.status !== "blank_explanation_opened") return;
    const context = custodyLedgerTransferContextRef.current;
    const submissionController = context
      ? createCustodyLedgerNormalExplanationSubmission(
        context.routeState,
        context.primaryResult,
        context.freshPracticeState,
    ...
    custodyLedgerExplanationSubmissionControllerRef.current = submissionController;
    custodyLedgerRAIPrimaryEntryControllerRef.current = null;
    custodyLedgerRAIPrimaryConvergenceControllerRef.current = null;
    custodyLedgerRAITransferConvergenceControllerRef.current = null;
    custodyLedgerTransferContextRef.current = {
      ...context,
      explanationEntryState: result.state,
    };
    setCustodyLedgerPrimaryView(submissionController.getState());
  }

  function handleCustodyLedgerExplanationSubmit(responses, event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerExplanationSubmissionControllerRef.current;
    if (!controller) return null;
    for (const [dimension, value] of Object.entries(responses)) {
      const update = controller.updateResponse({ dimension, value });
    ...
    const result = controller.dispatch({
      ...
      activationKind: routeActivationKind(event),
      eventToken: routeEventToken("rp002-explanation-submit"),
    });
    if (result?.status === "feedback") {
      custodyLedgerRAIPrimaryEntryControllerRef.current = null;
      custodyLedgerRAIPrimaryConvergenceControllerRef.current = null;
    ...
    if (result?.status === "python_explanation_complete") {
      const context = custodyLedgerTransferContextRef.current;
      const raiPrimaryEntryController = context
        ? createCustodyLedgerNormalRAIPrimaryEntry(
          context.routeState,
          context.primaryResult,
          context.freshPracticeState,
        ...
        : null;
      if (!raiPrimaryEntryController) return null;
      custodyLedgerRAIPrimaryEntryControllerRef.current = raiPrimaryEntryController;
      custodyLedgerRAIPrimaryConvergenceControllerRef.current = null;
      custodyLedgerRAITransferConvergenceControllerRef.current = null;
      custodyLedgerTransferContextRef.current = {
        ...context,
        explanationCompleteState: result.state,
      };
      setCustodyLedgerPrimaryView(result.state);
    }
    return result;
  ...
  function retryCustodyLedgerExplanation() {
    const result = custodyLedgerExplanationSubmissionControllerRef.current?.retryBlank();
    if (result?.status === "blank_retry") {
      custodyLedgerRAIPrimaryEntryControllerRef.current = null;
      custodyLedgerRAIPrimaryConvergenceControllerRef.current = null;
      custodyLedgerRAITransferConvergenceControllerRef.current = null;
  ...
  function openCustodyLedgerRAIPrimary(event) {
    if (demoTour || typeof window === "undefined") return;
    const result = custodyLedgerRAIPrimaryEntryControllerRef.current?.dispatch({
      packetId: "RP-002",
      ...
      activationKind: routeActivationKind(event),
      eventToken: routeEventToken("rp002-open-rai-primary"),
    });
    if (result?.status !== "blank_rai_primary_opened") return;
    const context = custodyLedgerTransferContextRef.current;
    const convergenceController = context
      ? createCustodyLedgerNormalRAIPrimaryConvergence(
        context.routeState,
        context.primaryResult,
        context.freshPracticeState,
      ...
      : null;
    if (!convergenceController) return;
    custodyLedgerRAIPrimaryConvergenceControllerRef.current = convergenceController;
    custodyLedgerRAITransferConvergenceControllerRef.current = null;
    setCustodyLedgerPrimaryView(convergenceController.getState());
  }

  function custodyLedgerRAIConvergenceIntent(action, event, token) {
    return {
      packetId: "RP-002",
  ...
  function submitCustodyLedgerRAICase(responses, event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAIPrimaryConvergenceControllerRef.current;
    const current = controller?.getState();
    if (!controller || current?.phase !== "RAIC-00") return null;
    ...
    const result = controller.dispatch(custodyLedgerRAIConvergenceIntent(
      CUSTODY_LEDGER_SUBMIT_RAI_CASE,
      event,
      `rp002-rai-submit-${current.case.id.toLowerCase()}`,
    ));
    if (["next_blank_case", "actual_miss_feedback"].includes(result?.status)) {
      setCustodyLedgerPrimaryView(result.state);
    }
    if (result?.status === "strict_primary_complete") {
      const context = custodyLedgerTransferContextRef.current;
      const transferConvergenceController = context
        ? createCustodyLedgerNormalRAITransferConvergence(
          context.routeState,
          context.primaryResult,
          context.freshPracticeState,
  ...
  function acknowledgeCustodyLedgerRAIFeedback(event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAIPrimaryConvergenceControllerRef.current;
    const result = controller?.dispatch(custodyLedgerRAIConvergenceIntent(
      CUSTODY_LEDGER_ACKNOWLEDGE_RAI_FEEDBACK,
      event,
      "rp002-rai-feedback-acknowledge",
    ));
    if (result?.status === "zero_credit_guide_opened") setCustodyLedgerPrimaryView(result.state);
    return result;
  }

  function completeCustodyLedgerRAIGuide(responses, event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAIPrimaryConvergenceControllerRef.current;
    const current = controller?.getState();
    if (!controller || current?.phase !== "RAIC-30G") return null;
    for (const control of current.controls) {
      const update = controller.updateGuidedResponse({
        dimension: control.authorityDimension,
        value: responses[control.id] ?? "",
      });
      if (update.status !== "guided_response_updated_session_only") return null;
    }
    const result = controller.dispatch(custodyLedgerRAIConvergenceIntent(
      CUSTODY_LEDGER_COMPLETE_RAI_GUIDE,
      event,
      "rp002-rai-guide-complete",
    ));
  ...
  function custodyLedgerRAITransferConvergenceIntent(action, event, token) {
    return {
      packetId: "RP-002",
  ...
  function submitCustodyLedgerRAITransferCase(responses, event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAITransferConvergenceControllerRef.current;
    const current = controller?.getState();
    if (!controller || current?.phase !== "RAITC-00") return null;
    for (const control of current.controls) {
      const update = controller.updateResponse({
        caseId: current.case.id,
        dimension: control.authorityDimension,
    ...
    const result = controller.dispatch(custodyLedgerRAITransferConvergenceIntent(
      CUSTODY_LEDGER_SUBMIT_RAI_TRANSFER_CASE,
      event,
  ...
  function acknowledgeCustodyLedgerRAITransferFeedback(event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAITransferConvergenceControllerRef.current;
    const result = controller?.dispatch(custodyLedgerRAITransferConvergenceIntent(
      CUSTODY_LEDGER_ACKNOWLEDGE_RAI_TRANSFER_FEEDBACK,
      event,
      "rp002-rai-transfer-feedback-acknowledge",
    ));
    if (result?.status === "zero_credit_transfer_guide_opened") setCustodyLedgerPrimaryView(result.state);
    return result;
  }

  function completeCustodyLedgerRAITransferGuide(responses, event) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAITransferConvergenceControllerRef.current;
    const current = controller?.getState();
    if (!controller || current?.phase !== "RAITC-30G") return null;
    for (const control of current.controls) {
      const update = controller.updateGuidedResponse({
        dimension: control.authorityDimension,
        value: responses[control.id] ?? "",
    ...
    const result = controller.dispatch(custodyLedgerRAITransferConvergenceIntent(
      CUSTODY_LEDGER_COMPLETE_RAI_TRANSFER_GUIDE,
      event,
      "rp002-rai-transfer-guide-complete",
    ));
  ...
  function createNormalCustodyLedgerPrepareSaveController(acceptedReviewState) {
    const context = custodyLedgerTransferContextRef.current;
    return context
      ? createCustodyLedgerNormalRAIPrepareSaveConfirmation(
        context.routeState,
  ...
  function createNormalCustodyLedgerAtomicSaveController(acceptedConfirmationState) {
    const context = custodyLedgerTransferContextRef.current;
    return context
      ? createCustodyLedgerNormalRAIAtomicSaveCommit(
        context.routeState,
  ...
  function prepareCustodyLedgerSaveConfirmation(event) {
    ...
    if (result?.status === "local_confirmation_visible") {
      const atomicController = createNormalCustodyLedgerAtomicSaveController(result.state);
      if (!atomicController) return null;
      custodyLedgerRAIAtomicSaveCommitControllerRef.current = atomicController;
      setCustodyLedgerPrimaryView(atomicController.getState());
    }
    return result;
  }
  ...
  function cancelCustodyLedgerSaveConfirmation(event) {
    ...
    if (result?.status === "confirmation_cancelled_write_free") {
      custodyLedgerRAIAtomicSaveCommitControllerRef.current = null;
      setCustodyLedgerPrimaryView(result.state);
    }
    return result;
  ...
  function custodyLedgerRAIAtomicSaveIntent(action, event, token) {
    return {
      packetId: "RP-002",
      version: CUSTODY_LEDGER_RAI_ATOMIC_SAVE_VERSION,
      mode: "campaign",
      owner: action === CUSTODY_LEDGER_SAVE_INTENT
  ...
  function dispatchCustodyLedgerAtomicSave(action, event, token) {
    if (demoTour || typeof window === "undefined") return null;
    const controller = custodyLedgerRAIAtomicSaveCommitControllerRef.current;
    const result = controller?.dispatch(custodyLedgerRAIAtomicSaveIntent(
      action,
      event,
      token,
    ));
  ...
  function commitCustodyLedgerAtomicSave(event) {
    return dispatchCustodyLedgerAtomicSave(
      CUSTODY_LEDGER_SAVE_INTENT,
      event,
      "rp002-save-bounded-comparison",
    );
  }

  function retryCustodyLedgerAtomicSave(event) {
    return dispatchCustodyLedgerAtomicSave(
      CUSTODY_LEDGER_RETRY_SAVE,
      event,
      "rp002-retry-save",
  ...
  function returnSafelyFromCustodyLedgerAtomicSave(event) {
    return dispatchCustodyLedgerAtomicSave(
      CUSTODY_LEDGER_RETURN_SAFELY,
      event,
      "rp002-return-safely",
  ...
  function useHotspot(hotspotId = scene.primaryHotspotId ?? "primary") {
    if(scene.id==="ruins"&&hotspotId==="crown-counterpoise"){if(!crownCounterpoiseNativeActive||terminalOpen)return;if(verb==="LOOK AT"){setDialogue(CROWN_COUNTERPOISE_COPY.look,"scene");setSceneAnnouncement(CROWN_COUNTERPOISE_COPY.look);return;}if(verb==="TALK TO"){setDialogue(CROWN_COUNTERPOISE_COPY.talk,"pilot");setSceneAnnouncement(CROWN_COUNTERPOISE_COPY.talk);return;}if(crownCounterpoiseState.lesson==="capstone-readiness")openCapstoneReadiness();else if(crownCounterpoiseState.lesson==="mixed-simulation")openMixedSimulation();else if(crownCounterpoiseState.lesson==="final-confidence")openFinalConfidence();else{const message=`${CROWN_COUNTERPOISE_COPY.complete} ${CROWN_COUNTERPOISE_COPY.next}`;setDialogue(message,"system");setSceneAnnouncement(message);}return;}
    if(scene.id==="ruins"&&hotspotId==="waterline-ledger"){if(!waterlineLedgerNativeActive||terminalOpen)return;if(verb==="LOOK AT"){setDialogue(WATERLINE_LEDGER_COPY.look,"scene");setSceneAnnouncement(WATERLINE_LEDGER_COPY.look);return;}if(verb==="TALK TO"){setDialogue(WATERLINE_LEDGER_COPY.talk,"pilot");setSceneAnnouncement(WATERLINE_LEDGER_COPY.talk);return;}if(waterlineLedgerState.state==="complete"){const message=`${WATERLINE_LEDGER_COPY.mastered} ${WATERLINE_LEDGER_COPY.next_boundary}`;setDialogue(message,"system");setSceneAnnouncement(message);return;}if(waterlineLedgerState.lesson==="remediation-planner")openRemediationPlanner();else openObjectiveLedger();return;}
    if(scene.id==="ruins"&&hotspotId==="tidal-pattern-loom"){if(tidalPatternLoomState==="hidden"||terminalOpen)return;if(verb==="LOOK AT"){setDialogue(TIDAL_PATTERN_LOOM_COPY.unseen,"scene");return;}if(verb==="TALK TO"){setDialogue("Complete silence.","pilot");return;}if(tidalPatternLoomState==="complete"){setDialogue(`${TIDAL_PATTERN_LOOM_COPY.mastered} ${TIDAL_PATTERN_LOOM_COPY.nextBoundary}`,"system");return;}if(textSpeechPatternEvidence?.masteryStatus==="mastered")openVisualPatterns();else openTextSpeechPatterns();return;}
  ...
  function acknowledgeTextAnalysisPrimary() { if (!textAnalysisSession?.complete || !textAnalysisEvidence?.confidence) return; floodedChoirFocusPendingRef.current=true; setTextAnalysisEvidence((previous) => updateTextAnalysisEvidence(previous, { form: "transfer", masteryStatus: "primary_complete", clearMisconceptionTags: true })); setTextAnalysisSession(null); setTerminalOpen(false); setRuinsTerminalKind(null); setDialogue("Text Analysis primary form complete at 12 of 12. Fresh transfer and closed-note workload choice remain.", "teacher"); }
  ...
  function acknowledgeTextAnalysisMastery() { if (!textAnalysisSession?.explanationResult?.passed || !textAnalysisSession.ownershipConfirmed || !textAnalysisEvidence?.confidence) return; floodedChoirFocusPendingRef.current=true; setTextAnalysisEvidence((previous) => updateTextAnalysisEvidence(previous, { form: "explanation", masteryStatus: "mastered", clearMisconceptionTags: true })); setTextAnalysisSession(null); setTerminalOpen(false); setRuinsTerminalKind(null); setDialogue("Text Analysis mastery confirmed: both 12-of-12 forms and closed-note workload explanation are complete.", "teacher"); }
  ...
  function acknowledgeSingleAgentMastery(){if(!singleAgentSession?.explanationResult?.passed||!singleAgentSession.ownershipConfirmed||!singleAgentEvidence?.confidence)return;focusContinueAfterSingleAgentRef.current=true;drownedSwitchyardFocusPendingRef.current=true;tidalPatternLoomTransitionPendingRef.current=TIDAL_PATTERN_LOOM_REGISTRY.source.enabled===true;setSingleAgentEvidence(previous=>updateSingleAgentEvidence(previous,{form:"explanation",masteryStatus:"mastered",clearMisconceptionTags:true}));setSingleAgentSession(null);setTerminalOpen(false);setRuinsTerminalKind(null);setDialogue("Single Agent mastery confirmed: both 12-of-12 forms and the closed-note fit, privilege, failure-safety, and client-flow explanation are complete.","teacher");}
  ...
  function acknowledgeObjectiveLedgerPrimary(){if(!objectiveLedgerSession?.complete||!objectiveLedgerEvidence?.confidence)return;waterlineLedgerFocusPendingRef.current=waterlineLedgerNativeActive;setObjectiveLedgerEvidence(previous=>updateObjectiveLedgerEvidence(previous,{form:"transfer",masteryStatus:"primary_complete",clearMisconceptionTags:true}));setObjectiveLedgerSession(null);setTerminalOpen(false);setRuinsTerminalKind(null);setDialogue("Objective Ledger primary form complete at 30 of 30. Fresh transfer and closed-note domain mastery remain; confidence is not mastery.","teacher");}
  ...
  function acknowledgeObjectiveLedgerMastery(){if(!objectiveLedgerSession?.explanationResult?.passed||!objectiveLedgerSession.ownershipConfirmed||!objectiveLedgerEvidence?.confidence)return;focusContinueAfterObjectiveLedgerRef.current=true;waterlineLedgerFocusPendingRef.current=waterlineLedgerNativeActive;setObjectiveLedgerEvidence(previous=>updateObjectiveLedgerEvidence(previous,{form:"explanation",masteryStatus:"mastered",clearMisconceptionTags:true}));setObjectiveLedgerSession(null);setTerminalOpen(false);setRuinsTerminalKind(null);setDialogue("Objective Ledger mastery confirmed: all 15 objectives are ready from 30-of-30 primary and transfer evidence plus closed-note domain mastery. This course evidence is not an exam guarantee.","teacher");}
  ...
  function acknowledgeRemediationPlannerPrimary(){if(!remediationPlannerSession?.complete||!remediationPlannerEvidence?.confidence)return;waterlineLedgerFocusPendingRef.current=waterlineLedgerNativeActive;setRemediationPlannerEvidence(previous=>updateRemediationPlannerEvidence(previous,{form:"transfer",masteryStatus:"primary_complete",clearMisconceptionTags:true}));setRemediationPlannerSession(null);setTerminalOpen(false);setRuinsTerminalKind(null);setDialogue("Primary planner complete: 12/12. Fresh transfer and a closed-note explanation remain. Confidence is not evidence or an exam guarantee.","teacher");}
  ...
  function checkFinalConfidenceEntry(event){event.preventDefault();const form=event.currentTarget,entryGate=evaluateFinalConfidenceEntryGate(finalConfidenceSession.entryDraft);setFinalConfidenceEvidence(p=>updateFinalConfidenceEvidence(p,{entryEvidence:finalConfidenceSession.entryDraft,masteryStatus:entryGate.ready?"in_progress":"entry_gate_incomplete"}));setFinalConfidenceSession({...finalConfidenceSession,entryGate,phase:entryGate.ready?"items":"gate",response:{decision:"",reason:""},result:null});requestAnimationFrame(()=>{const target=entryGate.ready?finalConfidenceTimerRef.current:form.querySelector('[aria-invalid="true"]');target?.scrollIntoView({block:"nearest"});target?.focus({preventScroll:true});});}
  ...
  function retryFinalConfidenceItem(){setFinalConfidenceSession({...finalConfidenceSession,response:{decision:"",reason:""},result:null});requestAnimationFrame(()=>finalConfidenceFirstSelectRef.current?.focus({preventScroll:true}));}
  function advanceFinalConfidence(){if(!finalConfidenceSession.result?.passed)return;if(finalConfidenceSession.index===finalConfidenceItems.length-1){setFinalConfidenceSession({...finalConfidenceSession,complete:true});requestAnimationFrame(()=>finalConfidenceSummaryHeadingRef.current?.focus({preventScroll:true}));return;}setFinalConfidenceSession({...finalConfidenceSession,index:finalConfidenceSession.index+1,response:{decision:"",reason:""},result:null,hintLevel:0});requestAnimationFrame(()=>finalConfidenceItemHeadingRef.current?.focus({preventScroll:true}));}
  ...
  if (mode === "rp002-arrival") {
    ...
    return (
      <CivicRecordArrival
        ...
        onExplanationSubmit={handleCustodyLedgerExplanationSubmit}
        onExplanationRetry={retryCustodyLedgerExplanation}
        onRAIPrimaryOpen={openCustodyLedgerRAIPrimary}
        onRAIPrimarySubmit={submitCustodyLedgerRAICase}
        onRAIFeedbackAcknowledge={acknowledgeCustodyLedgerRAIFeedback}
        onRAIGuideComplete={completeCustodyLedgerRAIGuide}
      ...
      <section className="scene-frame" aria-label={`${scene.location} scene`}>
        ...
        {scene.id === "meadow" ? (
          ...
          <>
            {scene.id === "ruins" ? (
              crownCounterpoiseNativeActive ? (
                <img className="scene-art waterline-ledger-art crown-counterpoise-art" src={crownCounterpoiseImage} alt={CROWN_COUNTERPOISE_COPY.alt} data-crown-counterpoise-source={CROWN_COUNTERPOISE_REGISTRY.source.path} />
              ) : waterlineLedgerNativeActive ? (
                <img className="scene-art waterline-ledger-art" src={waterlineLedgerImage} alt={WATERLINE_LEDGER_COPY.alt} data-waterline-ledger-source={WATERLINE_LEDGER_REGISTRY.source.path} />
              ) : tidalPatternLoomActive ? (
                <img className="scene-art tidal-pattern-loom-art" src={tidalPatternLoomImage} alt={TIDAL_PATTERN_LOOM_COPY.alt} data-tidal-pattern-loom-source={TIDAL_PATTERN_LOOM_REGISTRY.source.path} />
            ...
            {hotspotButtons}
            {waterlineLedgerNativeActive&&<><p id="waterline-ledger-physical-description" className="sr-only">{WATERLINE_LEDGER_COPY.available}</p><p id="waterline-ledger-state-description" className="sr-only">{getWaterlineLedgerCopyForState(waterlineLedgerState)}</p></>}
            {crownCounterpoiseNativeActive&&<><p id="crown-counterpoise-physical-description" className="sr-only">{CROWN_COUNTERPOISE_COPY.available}</p><p id="crown-counterpoise-state-description" className="sr-only">{getCrownCounterpoiseCopyForState(crownCounterpoiseState)}</p></>}
          </>
        )}
        ...
        {terminalOpen&&scene.id==="ruins"&&ruinsTerminalKind==="final-confidence"&&finalConfidenceSession&&(
          <TerminalShell exerciseId={finalConfidenceExercise.exercise_id} title="AI-901 Final Confidence" filename={finalConfidenceSession.phase==="gate"?"entry_evidence.json":"sim_03_final_confidence.json"} lessonId="SIM-03" statusText={finalConfidenceSession.phase==="gate"?"ENTRY GATE":finalConfidenceSession.complete?"24/24 COMPLETE":`ITEM ${finalConfidenceSession.index+1}/12`} closeLabel="Exit Final Confidence" describedBy={finalConfidenceDialogDescribedBy} restoreFocusTo={terminalTriggerRef.current} onClose={exitFinalConfidence}>
            <section className="model-choice-workspace final-confidence-workspace">
              ...
              {finalConfidenceSession.phase==="gate"?<form className="model-choice-form final-confidence-gate" onSubmit={checkFinalConfidenceEntry} noValidate><header><p className="pane-label">SYSTEM // ENTRY EVIDENCE GATE</p><h2>Confirm the bounded practice record</h2><p>Use only your non-sensitive course record. If a fact is missing, leave the gate incomplete and return after fresh practice.</p></header><div className="model-choice-fields">
                <label><span>L-06-03 readiness state</span><select aria-label="Final confidence L-06-03 readiness" {...finalConfidenceEntryA11y(finalConfidenceSession.entryGate,"l0603Readiness")} value={finalConfidenceSession.entryDraft.l0603ReadinessState} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,entryDraft:{...finalConfidenceSession.entryDraft,l0603ReadinessState:e.target.value},entryGate:null})}><option value="not_ready">Not yet ready</option><option value="ready_for_next_practice_checkpoint">Ready for next practice checkpoint</option></select></label>
                {[['cum01TransferScore','CUM-01 transfer score',16,'cum01Transfer'],['sim01Score','SIM-01 score',24,'sim01'],['sim02Score','SIM-02 score',24,'sim02'],['priorSimulationSeparationHours','Prior simulation separation hours',48,'spacing'],['openCriticalMisconceptions','Open critical misconceptions',0,'criticalMisconceptions']].map(([key,label,max,check])=><label key={key}><span>{label}</span><input aria-label={`Final confidence ${label}`} {...finalConfidenceEntryA11y(finalConfidenceSession.entryGate,check)} type="number" min="0" max={key.includes('Score')?max:undefined} value={finalConfidenceSession.entryDraft[key]} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,entryDraft:{...finalConfidenceSession.entryDraft,[key]:Number(e.target.value)},entryGate:null})}/></label>)}
                <label><span>Official sources reverified on</span><input aria-label="Final confidence official sources reverified on" {...finalConfidenceEntryA11y(finalConfidenceSession.entryGate,"sourceFreshness")} type="date" value={finalConfidenceSession.entryDraft.officialSourcesReverifiedOn} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,entryDraft:{...finalConfidenceSession.entryDraft,officialSourcesReverifiedOn:e.target.value},entryGate:null})}/></label>
                <label><span>Simulation attempted on</span><input aria-label="Final confidence attempted on" {...finalConfidenceEntryA11y(finalConfidenceSession.entryGate,"sourceFreshness")} type="date" value={finalConfidenceSession.entryDraft.attemptedOn} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,entryDraft:{...finalConfidenceSession.entryDraft,attemptedOn:e.target.value},entryGate:null})}/></label>
              </div><label className="ownership-confirmation"><input type="checkbox" {...finalConfidenceEntryA11y(finalConfidenceSession.entryGate,"highConfidenceRetest")} checked={finalConfidenceSession.entryDraft.highConfidenceMissesRetested} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,entryDraft:{...finalConfidenceSession.entryDraft,highConfidenceMissesRetested:e.target.checked},entryGate:null})}/>Every high-confidence miss was explained and retested on fresh practice.</label><section className="terminal-console model-choice-output"><div className="console-heading-row"><strong>SYSTEM // PREREQUISITE VALIDATOR</strong><button className="run-action" type="submit">Check entry evidence</button></div><div role="status" aria-live="polite" aria-atomic="true">{finalConfidenceSession.entryGate?finalConfidenceSession.entryGate.ready?"ENTRY GATE PASS · Final Confidence unlocked.":`ENTRY GATE INCOMPLETE · ${finalConfidenceSession.entryGate.failed.length} ${finalConfidenceSession.entryGate.failed.length===1?"prerequisite needs":"prerequisites need"} attention.`:"Enter the non-sensitive gate facts; no exam or service data is requested."}</div>{finalConfidenceSession.entryGate&&!finalConfidenceSession.entryGate.ready&&<><ul className="final-confidence-entry-errors">{finalConfidenceSession.entryGate.failed.map(check=><li key={check} id={`final-confidence-entry-${check}-feedback`}>{FINAL_CONFIDENCE_ENTRY_ERRORS[check]??"Complete the missing entry evidence."}</li>)}</ul><p className="teacher-remediation"><strong>901 TEACHER // ENTRY REMEDIATION</strong><span>Complete the named prerequisite, preserve a fresh retest, or reverify the current official source before attempting this final form. The gate cannot be bypassed by confidence.</span></p></>}</section></form>
              :<><label className="ownership-confirmation"><input ref={finalConfidenceTimerRef} type="checkbox" checked={finalConfidenceSession.timingEnabled} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,timingEnabled:e.target.checked})}/>Enable optional 25-minute diagnostic timer</label><p role="timer">{finalConfidenceSession.timingEnabled?`Diagnostic elapsed: ${finalConfidenceSession.elapsedSeconds} seconds. No time limit; timing cannot change mastery.`:"Untimed mode active · fully equivalent for strict mastery."}</p>{finalConfidenceSession.complete?<section className="workload-summary"><p className="pane-label">901 TEACHER // FINAL CONFIDENCE COMPLETE</p><h2 ref={finalConfidenceSummaryHeadingRef} tabIndex="-1">24 / 24 dimensions</h2><p>All fifteen objectives are covered after a valid entry gate. This supports only a cautious scheduling recommendation, never an exam guarantee.</p><fieldset className="confidence-group"><legend>Confidence (separate from correctness)</legend>{["low","medium","high"].map(v=><label key={v}><input type="radio" name="final-confidence-confidence" checked={finalConfidenceEvidence?.confidence===v} onChange={()=>setFinalConfidenceEvidence(p=>updateFinalConfidenceEvidence(p,{confidence:v}))}/>{v}</label>)}</fieldset><button className="confirm-action" type="button" disabled={!finalConfidenceEvidence?.confidence} onClick={acknowledgeFinalConfidenceMastery}>Acknowledge final confidence evidence</button></section>:(()=>{const item=finalConfidenceItems[finalConfidenceSession.index],options=getFinalConfidenceOptions(item.id),feedback=getFinalConfidenceFeedback(item,finalConfidenceSession.result);return <form className="model-choice-form" onSubmit={checkFinalConfidence}><header><p className="pane-label">CLOSED NOTE · PILOT // FINAL CONFIDENCE OWNER · {item.id} · {item.domain} · {finalConfidenceSession.index+1}/12</p><h2 ref={finalConfidenceItemHeadingRef} tabIndex="-1">{item.prompt}</h2><p>Objective tags: {item.objective_ids.join(" · ")}</p></header><div className="model-choice-fields">{finalConfidenceDimensions.map(d=>{const ok=finalConfidenceSession.result?.correctness[d],id=`final-confidence-${d}-feedback`;return <label key={d}><span>{d}</span><select ref={d===finalConfidenceDimensions[0]?finalConfidenceFirstSelectRef:null} aria-label={`Final confidence ${d}`} aria-invalid={finalConfidenceSession.result?!ok:undefined} aria-describedby={finalConfidenceSession.result?id:undefined} value={finalConfidenceSession.response[d]} onChange={e=>setFinalConfidenceSession({...finalConfidenceSession,response:{...finalConfidenceSession.response,[d]:e.target.value},result:null})}><option value="">Choose one</option>{options[d].map(v=><option key={v} value={v}>{formatChoice(v)}</option>)}</select>{finalConfidenceSession.result&&<small id={id}>{ok?"SYSTEM // Correct.":"901 TEACHER // Repair this objective boundary before retry."}</small>}</label>;})}</div><section className="terminal-console model-choice-output"><div className="console-heading-row"><strong>SYSTEM // STRICT 24-DIMENSION VALIDATOR</strong><button className="run-action" type="submit" disabled={finalConfidenceDimensions.some(d=>!finalConfidenceSession.response[d])}>Check final confidence item</button></div><div role="status" aria-live="polite" aria-atomic="true">{feedback.systemScore}</div>{feedback.teacherRemediation&&<p className="teacher-remediation"><strong>901 TEACHER // TARGETED OBJECTIVE REMEDIATION</strong><span>{feedback.teacherRemediation}</span></p>}{finalConfidenceSession.result&&!finalConfidenceSession.result.passed&&<button className="hint-action" type="button" onClick={retryFinalConfidenceItem}>Retry with blank choices</button>}{finalConfidenceSession.result?.passed&&<button className="confirm-action" type="button" onClick={advanceFinalConfidence}>{finalConfidenceSession.index===11?"View final confidence result":"Next final confidence item"}</button>}</section></form>;})()}</>}</section>
          </TerminalShell>)}
      ...
      <section className="command-panel" data-meadow-departure-choice={showMeadowDepartureChoice ? "true" : undefined} aria-label="Adventure controls and dialogue" inert={terminalOpen || demoTourConfirmation ? true : undefined}>
        ...
        <div className="dialogue-box" aria-live="polite">
          {questionOpen ? (
            ...
            <div className={showMeadowDepartureChoice ? "dialogue-copy meadow-departure-choice" : "dialogue-copy"} data-final-confidence-action={showFinalConfidenceAction ? "true" : undefined}>
              <p
