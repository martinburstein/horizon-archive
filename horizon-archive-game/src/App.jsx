...
export function App() {
  ...
  const activeSceneHotspots = scene.id === "ruins" && strandedLensCradleActive
    ...
    : sceneHotspots;
  const presentedSceneHotspots = waterlineLedgerNativeActive
  ...
  useLayoutEffect(() => {if(!focusContinueAfterRemediationPlannerRef.current||terminalOpen||!pendingAdvance||remediationPlannerEvidence?.masteryStatus!=="mastered")return;focusContinueAfterRemediationPlannerRef.current=false;const target=waterlineLedgerNativeActive?waterlineLedgerRef.current:continueButtonRef.current;if(target?.isConnected)target.focus({preventScroll:true});if(waterlineLedgerNativeActive)setSceneAnnouncement(WATERLINE_LEDGER_COPY.mastered);},[terminalOpen,pendingAdvance,remediationPlannerEvidence?.masteryStatus,waterlineLedgerNativeActive]);
          ...
          <>
            {scene.id === "ruins" ? (
              waterlineLedgerNativeActive ? (
                <img className="scene-art waterline-ledger-art" src={WATERLINE_LEDGER_SOURCE_URL} alt={WATERLINE_LEDGER_COPY.alt} data-waterline-ledger-source={WATERLINE_LEDGER_REGISTRY.source.path} />
