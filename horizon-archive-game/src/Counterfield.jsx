...
export function Counterfield({ state, onAction, onFieldChange }) {
  ...
  const recoveryFocusId = state.focusIntent?.target ?? "cf20-python-primary-first-failed";
  return (
    <main ref={rootRef} className="counterfield-shell" data-product-landmark="counterfield-product-landmark" aria-labelledby={state.headingId}>
      ...
      <section className="counterfield-panel">
        ...
        {group === "cf20_recovery" && <section><p id={recoveryFocusId} tabIndex="-1">First incomplete public dimension: {state.failedPublicIds?.[0] ?? "current responsibility"}</p><Action action={counterfieldActions.retry} onAction={onAction} /></section>}
