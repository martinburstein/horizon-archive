...
- five representative closed fixture states across 20 exact DPR-1
  ...
  `>=44px` controls, heading focus, one polite atomic status, reachable
  actions, console/page silence, and local-only requests;
- forced-colors and reduced-motion emulation passed on route-ready and the
  high-density TR-30 transfer state across all four layouts;
- fixture identity is excluded from production;
...
- Preserve closed canon, intended ending, strict independent evidence/no
  cross-credit, privacy, accessibility, offline/no-authority/
  no-exam-guarantee, no-credit Tour, deterministic save/recovery, invariant
  SC-04/SC-05, and the hard stop at TR-40 plus only the two known returns and
  optional destinationless record.
- No traversable common return, direct Civic Comparison shortcut, RP-005,
