...
4. `SC-03-30 — Changed expedition state: bounded comparison review`
   - Reuse the overview world plate and the shared continuous animation clock. The compact human expedition interface outside the world plate holds the three human labels and save action.
   - `SAVE BOUNDED COMPARISON` advances to `SC-03-40`; cancel returns to the unchanged overview. No request is sent to the closed layer.
...
- Next pass: Curriculum Checker Agent performs a narrow A3 impact review on this same packet without reopening a `SOLIDIFIED` mapping unless the new runtime copy changes scored evidence, remediation, transfer, privacy, or authority boundaries.

### Exact Curriculum Checker handoff

Curriculum Checker Agent: perform only the A3 impact review caused by the responsive-composition update. Confirm that retiring exact `640 x 480` / `320 x 240` acceptance and allowing owner/message/action groups to reflow does not change the already `SOLIDIFIED` `PY-009` dictionary contract, `RP002-RAI-01` responsible-AI contract, strict primary/remediation/blank-transfer evidence, or prerequisite gates. Preserve every visible owner prefix and complete no-request/no-authority clause, the locked `SC-03-00 -> 10 -> 20 -> 30 -> 40` sequence, `SC-03-50` return, `cityThresholdAnchorRecorded=true`, `civicDistrictRouteAvailable=true`, unchanged `continuation`, `city_state_delta=None`, reversible City Threshold return, cleared working notes, closed identity-bearing layer, invariant photoreal world, no external request/action, and the distinction between human validation feedback and city response. Treat viewport size, responsive crop, interface placement, wrapping, focus, color, animation, navigation, and world observation as zero mastery evidence. If the mapping remains valid, record `NO REOPEN — SOLIDIFIED` and hand the responsive ownership/layout lock to Gameplay Master without rewriting settled lesson content.
