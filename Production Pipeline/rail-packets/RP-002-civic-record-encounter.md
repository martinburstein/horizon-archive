...
The `SC-03-30` comparison and save controls belong to the existing compact `640 x 120` human interface band, with a purpose-authored narrow layout. They are never painted, projected, reflected, or diegetically attached to the `640 x 360` Builder world.
...
- At desktop, the `640 x 120` interface band reserves a compact owner line, a message region supporting two wrapped text lines, and a bottom action row with `44 px` minimum targets. Only the longest A1 lines use two message lines; semantic line breaks fall after sentence or clause boundaries and do not delete `no local request`, `cannot claim access`, provisional human ownership, closed-record status, or unchanged `continuation` language.
- At the exact `320 x 240` narrow derivative, the `320 x 180` world remains unobscured and the interface begins below it in document flow. The purpose-authored interface may extend below the first viewport and scroll vertically; it may not clip, shrink below the readable type contract, cover Builder matter, or compress action targets below `44 px`. Focus arrival scrolls the full owner/message/action group into view. The longest messages may wrap to four lines, with the same semantic clause boundaries and complete wording.
...
- Desktop and narrow layouts retain a visible no-request/no-authority clause where applicable, the Pilot-owned provisional label, the closed-record statement, and `continuation` unchanged meaning without relying on color. No interface state implies that the city refused, approved, remembered, answered, or received anything.
...
- Runtime ownership impact validation: `PASS` — every A1 Custody Ledger line remains wholly inside the human interface, keeps its visible text owner, fits the desktop two-line message region or the purpose-authored scroll-safe narrow derivative, preserves `44 px` actions, and creates no world or story-state delta.
...
Curriculum Checker Agent: perform only the A3 impact review caused by the new Custody Ledger Runtime Ownership Copy. Confirm that the visible owner prefixes and the prerequisite, validation, feedback, save, restore, cancel, and Demo Tour lines do not change the already `SOLIDIFIED` `PY-009` dictionary contract, `RP002-RAI-01` responsible-AI contract, strict primary/remediation/blank-transfer evidence, or prerequisite gates. Preserve the locked `SC-03-00 -> 10 -> 20 -> 30 -> 40` sequence, `SC-03-50` return, `cityThresholdAnchorRecorded=true`, `civicDistrictRouteAvailable=true`, unchanged `continuation`, `city_state_delta=None`, reversible City Threshold return, cleared working notes, closed identity-bearing layer, invariant photoreal world, no external request/action, and the distinction between human validation feedback and city response. Treat interface layout, wrapping, focus, color, animation, navigation, and world observation as zero mastery evidence. If the mapping remains valid, record `NO REOPEN — SOLIDIFIED` and hand the exact copy ownership/layout lock to Gameplay Master without rewriting settled lesson content.
