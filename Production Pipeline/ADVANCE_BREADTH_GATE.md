...
The coordinator promoted RP-003 and authorized the contiguous shell. RP-004's orientation cycle retains one bounded expansion path: arrive from the exact RP-003 end state, deliberately observe three physical current/corridor relations plus one apparent common return, complete one real-Python occupational correspondence over sanitized replicas only, save an expedition-owned form/corridor/purpose-unknown note, and leave the live landscape unchanged. A1 closed Story; A2 closed Scene and the production-art-plan gate with one invariant `SC-05` panoramic master supporting five compact phases. A3–A5 must work this same packet in order; RP-005 remains reserved until RP-004 reaches `GATE REVIEW` or `READY FOR WORKING`. Additional lore or storyboard micro-seams are not authorized during this cycle.
