...
The coordinator promoted RP-004 and authorized the contiguous shell. RP-005 begins its orientation cycle with one bounded expansion path: arrive from the exact RP-004 end state, deliberately observe a recurring exposed range, bounded divergent traces, one sealed branch, and layered stewardship, complete one real-Python occupational summary over sanitized replicas only, save an expedition-owned recurrence/divergence/unavailable/limits note, and leave the live field unchanged. A1 closes only the Story gate. A2–A5 must work this same packet in order; RP-006 remains reserved until RP-005 reaches `GATE REVIEW` or `READY FOR WORKING`. Additional lore micro-seams are not authorized during this cycle.
