...
The coordinator promoted RP-002 and authorized the contiguous seed. RP-003's orientation cycle retains one bounded surface path: arrive from the exact RP-002 end state, observe two exposed corresponding sequences plus one sealed source boundary, complete one real-Python occupational comparison using exposed inputs only, save an expedition-owned correspondence/difference/unavailable note, and leave the world physically unchanged. A1 closed Story; A2 closed Scene and the production-art-plan gate with one invariant `SC-04` world master supporting four compact phases. A3–A5 must work this same packet in order; RP-004 remains reserved until RP-003 reaches `GATE REVIEW` or `READY FOR WORKING`. Additional lore or storyboard micro-seams are not authorized during this cycle.
