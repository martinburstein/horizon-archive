...
- **Exact result / fail-closed rule:** mount only the existing blank Scene-owned `SC-04 / CM-00 ARRIVE + IDLE` group, retain `continuation`, keep `cityStateDelta=null`, and expose only existing orient/sealed-boundary/RP-002-return/City-Threshold-return actions. Invalid, Tour-derived, private, partial, stale, forged, or contaminated entry clears transients and returns to the exact verified RP-002 boundary. No CM-10, RP-004, RP-013, successor, access, authority, or world change is defined.
...
- **Authoritative evidence:** independent full `674/674`, all `15/15` validators, production build, clean first-launch `113.2s` isolated E2E after port-`5174` HTTP `200` preflight, desktop `1920 x 1080` / narrow `390 x 844` in-app title-shell review, exact hashes, clean logs/overflow, QA cleanup, and coordinator-preview shutdown pass on `index-DRVkMZaH.js` / `index-BPvBS70Z.css`. Martin's storage/save and protected user paths remain untouched; no gated screenshot is claimed.
- **Authoritative next exact edge:** existing bounded review -> one explicit Pilot `PREPARE SAVE` -> existing contained local confirmation -> cancel/Escape write-free return to bounded review. Stop with confirmation visible; do not activate `SAVE BOUNDED COMPARISON`, persist, enter failure/retry-save, complete RP-002, expose `RP-003+`, or create any world/access/authority/external effect, RP-013, or successor.
- The W4/coordinator-next and prior-release bullets immediately below are superseded by this coordinator release block.

- **Current Working handoff:** `RP002-AESTH-015 W4 VERIFIED — COORDINATOR NEXT`. W4 reproduced no exact presentation residual and changed no production artifact. Focused `74/74`, full `674/674`, both applicable validator self-tests, build, immutable civic art and six canonical-reference hashes, no-reference-leakage checks, exact served HTTP/asset hashes and markers, one strict `4173` listener, cache-bypassed final refresh, and patch gates pass on `index-DRVkMZaH.js` / `index-BPvBS70Z.css`. Browser-backed gated viewport evidence is still unavailable and not claimed; browser storage/save remains untouched. Coordinator independently validates the bounded review endpoint and its hard stop before `PREPARE SAVE`.
...
- **Advance A4 lock / next exact edge:** `PASS — GRAPH LOCKED; PROTECTED A5 NEXT`. Existing authorities form one strict zero-evidence path: one private-free seven-modality/one-hit RAI-conclusion dismissal -> strict System eligibility sanitation only on finalized Python/RAI/five-observation conjunction -> bounded review, or deterministic first-incomplete return. A5 composes only that graph and stops with bounded review visible, before `PREPARE SAVE`, persistence, completion, `RP-003+`, world/access/authority/external effect, RP-013, or successor.
- **Advance A5 protected result / W1 next:** `REVISE — PROTECTED RG-00/10/20/30/U CONTROLLER COMPLETE`. Focused `9/9`, related `79/79`, both applicable validator self-tests, production build, unchanged accepted bundle identity, non-integration/purity/restore/leakage, and patch gates pass. W1 begins only at accepted live exact `RAIEC-20C` and reports the earliest normal dismissal/strict-eligibility/bounded-review gap; storage/save remains untouched and the hard stop stays before `PREPARE SAVE`.
...
- **Finding:** the three explanation controls reuse the generic two-column custody grid, so wide `EXS-00` renders as `2 + 1` rather than the locked single row of three equal peers. W4 may add only a state-specific three-column wide grid and focused regression while retaining one-column narrow/`200%` reflow. The canonical archive reveal remains non-integrated reference evidence; the runtime civic plate stays unchanged.
- **Evidence/limit:** candidate root/JS/CSS HTTP `200`; exact hashes/markers pass; focused `59/59`; canonical reference and runtime master original-size/hash checks pass. No Browser backend was available, so storage remained untouched and no direct gated viewport, focus-ring, console, or screenshot claim is made.
...
**Latest Advance continuity handoff (RP002-P1-014 / A3):** `PASS — NO REOPEN / SOLIDIFIED / A4 NEXT`. Exact visible `FT-20C` -> explicit Pilot-owned explanation-entry intent -> canonical genuinely blank Teacher group changes course navigation only. The frozen `PY-009` cases, answers, six checks, independent primary/transfer prerequisites, strict explanation dimensions/evaluator, blankness, privacy, zero evidence/effect, hard later-state stop, and no successor remain exact. DI-001's accepted City Threshold state remains unchanged.

**Latest Advance continuity handoff (RP002-P1-014 / A2):** `PASS — COMPACT BLANK-EXPLANATION ENTRY PRESENTATION LOCKED / A3 NEXT`. Over the unchanged civic plate, complete System `FT-20C` retains one explicit Pilot-owned action until valid intent directly opens only the existing canonical genuinely blank Teacher group. Whole-group replacement creates no carried work, explanation attempt/evidence, story/save/world/route/access/authority/RAI/later effect, or successor. DI-001's accepted City Threshold state remains unchanged.
...
## 2026-07-17 W1 one-ID-to-second-near finding - `d859440`

- **Disposition:** `BLOCKED - RP002-P1-004 OPEN / W2 BUG REPAIR NEXT.` The accepted first observation, acknowledgement, evidence return, replay-only one-ID state, route return, and one-ID resume remain stable. The normal one-ID group cannot begin a second observation.
- **Expected/actual:** expected one literal Recorded/replay-only control plus the two remaining canonical actions as equal literal Available controls. One replay should preserve the single ID; one fresh Available activation should retain that first fact, add only its matching second ID/Scene statement, report `Near evidence recorded: 2 of 3. The district remains unchanged.`, and expose zero-credit `RETURN TO EVIDENCE`. Actual normal state filters the action group to replay-only, so both Available controls and the `2 of 3` acknowledgement are absent. This is P1 because the packet cannot advance beyond one fact without regressing the accepted boundary.
- **Evidence boundary:** the preserved user save remains at early Glass Meadow; W1 did not inspect or overwrite storage and makes no direct one-ID traversal claim. Focused normal-route, protected second-near, semantic-hotspot, Tour, and photoreal checks pass **50/50**; the production bundle contains first-near labels but no `2 of 3` acknowledgement. The previous coordinator gate covers the accepted one-ID presentation, sanitation, resume, return, responsive/44px, first-person art, and zero cross-credit/delta.
- **W2 acceptance:** normally compose the existing second-near authority into one Recorded plus two Available controls, permit exactly one second observation and exact `2 of 3` acknowledgement, preserve replay/wrong/duplicate recovery and allowlisted two-ID resume, provide zero-credit evidence/route returns, rebuild/reload, and stop before a third observation, `SC-03-20+`, learning, completion, or RP-003+ exposure.
...
## 2026-07-17 W1 blank-to-first-near finding - `0a6d7fe`

- **Disposition:** `BLOCKED - RP002-P1-003 OPEN / W2 BUG REPAIR NEXT.` The accepted staged route and blank `SC-03-10` remain stable, but the normal blank action group exposes only `RETURN TO CITY THRESHOLD` and cannot begin one canonical observation.
- **Expected/actual:** expected `INSPECT FIXED TRACE`, `INSPECT LATER STEWARDSHIP`, and `INSPECT OUTLINED GAP` as equal System-available/Pilot-owned controls. One fresh exact activation should add only its matching ID and Scene statement, report `Near evidence recorded: 1 of 3. The district remains unchanged.`, and expose zero-credit `RETURN TO EVIDENCE`; wrong/duplicate input should recover without credit. Actual: all three controls and the acknowledgement are absent from App/main and the production bundle, so neither a first observation nor its recoverable wrong/duplicate path can be attempted. Severity is P1.
- **Evidence boundary:** the live user save remains at early Glass Meadow and was preserved without storage inspection or overwrite. Source/bundle inspection occurred only after that natural black-box attempt. Focused normal-route, protected first-near, semantic-hotspot, Tour, and photoreal checks pass **45/45**, proving the approved authorities remain intact but UI-free. Existing blank-boundary coordinator evidence continues to cover return, resume, sanitation, responsive containment, first-person art, and zero cross-credit/delta.
- **W2 acceptance:** normally compose the existing authorities into exactly three equal `>=44px` controls, permit exactly one first observation and exact `1 of 3` acknowledgement, preserve recoverable wrong/duplicate behavior and allowlisted blank/one-ID resume, provide zero-credit `RETURN TO EVIDENCE`, rebuild/reload, and stop before a second observation, `SC-03-20+`, learning, completion, or RP-003+ exposure.
...
- **Accepted presentation:** the exact three-near/zero-far boundary permits one explicit zero-credit `COMPARE SCALE` transition into blank `SC-03-20`. Both far peers remain equal, natively and assistively disabled, handler-free, and persistently labeled `INERT // ZERO CREDIT // NOT YET ACTIVE`; the separate route return remains enabled.
- **Independent validation:** full **504/504**, production build, RP-002 through RP-008 curriculum self-tests, patch integrity, complete title-to-credits E2E with `credits:true` and `runtimeErrors:false`, served root/JS/CSS HTTP `200`, and clean live review at `1920 x 1080` and `390 x 844`. Both layouts had zero overflow and no undersized controls; the narrow frame occupied 385 of 390 CSS px.
- **Safety:** incidental regenerated QA binaries were restored. The user's save remained at the title/early-campaign boundary and was not altered, so this acceptance relies on focused W2–W4 evidence and independent release gates rather than a fabricated direct gated traversal.
- **Next boundary:** W1 reports only the earliest exact gap toward one bounded first far observation. Local comparison, learning, completion, and RP-003+ remain closed.
