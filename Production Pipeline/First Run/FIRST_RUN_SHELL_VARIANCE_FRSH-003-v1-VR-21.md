# First Run Shell Variance Reissue - Robust Focused-Total Parsing

Variance ID: `FRSH-003-v1-VR-21`

Disposition: **`FIRST RUN SHELL READY / ONE ROBUST-TOTALS SIX-PATH
...
`FRSH-003-v1-VR-20` / `FRCE-003-v1-VR-20`
...
Mission independently classifies the VR-20 stop as **`REQUIRED CORRECTION /
EXECUTION CONTROL / OPEN`**. The exact focused process truthfully emitted
`68` tests, `68` pass, and `0` fail, but Node used Unicode information-symbol
markers (`ℹ`) while the Mission-authored wrapper required number-sign markers
(`#`). The wrapper therefore rejected truthful exact totals after the focused
process passed. This establishes no candidate, product, manifest, test, E2E,
validation, evidence, threshold, learning, save, route, world, media, or
ending defect.

Mission separately preserves **`UNAUTHORIZED DIVERGENCE / PROTECTED PATH
ENUMERATION / OPEN`** from VR-17. VR-20 introduced no new enumeration and
does not waive or merge that classification.

VR-20 is consumed. Its truthful focused process result and all earlier partial
passes may not be promoted, reused as current proof, or rerun under VR-20.
VR-21 authorizes exactly one fresh static/focused invocation. It preserves the
scalar revision-range construction and immediate tri-state exit handling, and
changes only the focused total assertions to accept a line-anchored `#` or
Unicode information marker (`ℹ`) before each exact total. The required totals
remain exactly `68 tests / 68 pass / 0 fail`; no arbitrary prefix, alternate
number, missing summary, or nonzero process exit is accepted.

The one invocation must continue after a successful focused assertion through
all three final six-path checks in the same PowerShell process: final
allowlisted worktree drift, final allowlisted staged drift, and final
allowlisted diff integrity. None may be omitted, split into another invocation,
or inferred from VR-20.
...
if ($focusedExit -ne 0 -or
    $focusedText -notmatch '(?m)^(?:#|\u2139) tests 68\s*$' -or
    $focusedText -notmatch '(?m)^(?:#|\u2139) pass 68\s*$' -or
    $focusedText -notmatch '(?m)^(?:#|\u2139) fail 0\s*$') {
...
and read its full profile, VR-07 and VR-12 through VR-21,
FRCE-003-v1-VR-12 through VR-20, current FRAB-003, current handoff, and the
...
On complete PASS, Combat issues **`ROBUST-TOTALS SIX-PATH STATIC-FOCUSED PASS
/ RETURN TO FRESH MISSION / FRCE-003-v1-VR-21`**. On failure, it issues
**`HOLD / ROBUST-TOTALS SIX-PATH STATIC-FOCUSED FAILURE / NO RERUN / RETURN TO
FRESH MISSION / FRCE-003-v1-VR-21`**. In either case it may write only that
...
execution, and one-E2E policy; `node --check`; exact focused `68/68` under the
line-anchored `#`-or-`ℹ` parser; final allowlisted worktree drift, final
allowlisted staged drift, and final allowlisted diff integrity checks after the
focused assertion in that same invocation; no enumeration; and a synchronized
return.
...
`HEAD == origin/main == b9a1cab9a2aee0db742b809c5d81fe0715cdec47`
with no tracked or staged drift reported; pre-existing unrelated untracked user
material remained untouched. Mission inspected only controlling documents and
authored this administrative variance/handoff. Mission ran no Node command,
test, validator, build, preview, browser, E2E, diagnostic, summary, verifier,
cleanup, media, or product action; did not inspect protected/user-state
contents; and changed no implementation or maturity control.

Mission Captain signs **`FIRST RUN SHELL READY / ONE ROBUST-TOTALS SIX-PATH
STATIC-FOCUSED VERIFICATION / FRSH-003-v1-VR-21`** while preserving stage and
...
--check`, and robust-total focused `68/68` invocation, including all three
post-focused final checks in that same invocation; issue one exact return and
