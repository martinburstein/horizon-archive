...
"@
  ...
  $production.Stage='PT12_MATERIALIZE_STAGE'
    ...
    [Console]::Out.WriteLine('HOST06_REVIEW_READY|ordinal=A1'+'|length='+$targetIdentity[4]+'|sha256='+$decodedSha)
    ...
    $decisionPattern='\AHOST06_REVIEW_V1\|ordinal=A1\|decision=(?<decision>ACCEPT|REJECT)\|technical=true\|physical=(?<physical>true|false)\|layouts=(?<layouts>true|false)\|accessibility=(?<accessibility>true|false)\|codes=(?<codes>NONE|PHY(?:-\d{2})(?:,PHY-\d{2})*|LAYOUT|ACCESSIBILITY)\n\z'
    ...
    [Console]::Out.WriteLine('HOST06_PRODUCTION_COMPLETE|attempt=A1'+'|length='+$targetIdentity[4]+'|sha256='+$decodedSha+'|helperRootAbsent=true|liveRootAbsent=true')
    break
} catch {
