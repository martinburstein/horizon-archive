...
try{
  ...
  if($carrierBytes.Length-ne 36405-or([BitConverter]::ToString($sha.ComputeHash($carrierBytes))).Replace('-','').ToLowerInvariant()-cne'efa0cdac052cd081bbe3e2caaa8b9c3c6a232ff95e252042447c447397121dec'){throw 'PARENT_CARRIER_IDENTITY'}
  foreach($byte in $carrierBytes){if($byte-gt 127){throw 'PARENT_CARRIER_ASCII'}}
  $carrier=$strictUtf8.GetString($carrierBytes)
  if($carrier.Length-ne 36405){throw 'PARENT_CARRIER_LENGTH'}
  ...
  if($launcherBytes.Length-ne 2639-or$launcherHash-cne'd144aa0c5705519ebd32bcca15c2066ea777cf59d2285da9d7d79e19f1c3c041'){throw 'PARENT_LAUNCHER_IDENTITY'}
