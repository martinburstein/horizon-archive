...
try{
  ...
  if($carrierBytes.Length-ne 36477-or([BitConverter]::ToString($sha.ComputeHash($carrierBytes))).Replace('-','').ToLowerInvariant()-cne'edbe01ea2a7a7b158f09c4882f66ba1a21db72ce5fdff01775608671b193df83'){throw 'PARENT_CARRIER_IDENTITY'}
  foreach($byte in $carrierBytes){if($byte-gt 127){throw 'PARENT_CARRIER_ASCII'}}
  $carrier=$strictUtf8.GetString($carrierBytes)
  if($carrier.Length-ne 36477){throw 'PARENT_CARRIER_LENGTH'}
  ...
  if($launcherBytes.Length-ne 2639-or$launcherHash-cne'a57a27bf72126832089e0b5fde1530d16c528482153446a10de905a0226b2d44'){throw 'PARENT_LAUNCHER_IDENTITY'}
