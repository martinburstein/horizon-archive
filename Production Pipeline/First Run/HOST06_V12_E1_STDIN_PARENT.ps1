...
try{
  ...
  if($carrierBytes.Length-ne 36230-or([BitConverter]::ToString($sha.ComputeHash($carrierBytes))).Replace('-','').ToLowerInvariant()-cne'639ca116dd083019bdcd63471beae7b9e352cf29c1ac9166aa8e8aa4f13f7739'){throw 'PARENT_CARRIER_IDENTITY'}
  foreach($byte in $carrierBytes){if($byte-gt 127){throw 'PARENT_CARRIER_ASCII'}}
  $carrier=$strictUtf8.GetString($carrierBytes)
  if($carrier.Length-ne 36230){throw 'PARENT_CARRIER_LENGTH'}
  ...
  if($launcherBytes.Length-ne 2639-or$launcherHash-cne'6af193c4bd1abac6605f90b603ddb6d5084f5a0c9f1001b7a7899c00f38e7a1d'){throw 'PARENT_LAUNCHER_IDENTITY'}
