...
try{
  ...
  if($carrierBytes.Length-ne 34948-or([BitConverter]::ToString($sha.ComputeHash($carrierBytes))).Replace('-','').ToLowerInvariant()-cne'48a0b3556db83ec5fa6266b3e248c763dd3ed50b009271ea0f39e432152f5392'){throw 'PARENT_CARRIER_IDENTITY'}
  foreach($byte in $carrierBytes){if($byte-gt 127){throw 'PARENT_CARRIER_ASCII'}}
  $carrier=$strictUtf8.GetString($carrierBytes)
  if($carrier.Length-ne 34948){throw 'PARENT_CARRIER_LENGTH'}
  ...
  if($launcherBytes.Length-ne 2639-or$launcherHash-cne'a05570a6d3c330bda5d87421c24d1944a20a856b1cb2c170a70e7044a8b0cfa3'){throw 'PARENT_LAUNCHER_IDENTITY'}
