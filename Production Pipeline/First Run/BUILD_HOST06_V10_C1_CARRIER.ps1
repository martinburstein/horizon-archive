...
$carrierPath = Join-Path $root 'HOST06_V10_C1_PRODUCTION_CARRIER.ps1'
$launcherPath = Join-Path $root 'HOST06_V10_C1_LAUNCHER.ps1'
$stdinParentPath = Join-Path $root 'HOST06_V10_C1_STDIN_PARENT.ps1'
...
$newHelper = 'C:\Users\marti\AppData\Local\Temp\horizon-archive-host06-native-identity-v10-ea8cf781-baba-4d6b-bf86-6424f54fab99'
$oldLive = 'C:\Users\marti\AppData\Local\Temp\horizon-archive-host06-api-6eae8313-1407-492d-af7c-675051ab8e08'
$newLive = 'C:\Users\marti\AppData\Local\Temp\horizon-archive-host06-api-v10-3ac708f4-5a20-466e-9c18-14cc66a82d59'
...
namespace HorizonArchive.Host06V10 {
...
$transport = @'
  ...
  $production.Ordinal='C1'
  $production.Stage='PT09_SEND_ENTRY'
  $stagePath=$liveRoot+'\.attempt-C1-7eb25013-d4b3-46d9-ac8e-c49fd7242161.stage'
  $targetPath=$liveRoot+'\attempt-C1.png'
  $decisionPath=$liveRoot+'\.attempt-C1.review-v1'
  ...
  try{[HorizonArchive.Host06V10.StrictJson]::Validate($responseText);$responseObject=$serializer.DeserializeObject($responseText)}catch{throw 'PT10_RESPONSE_ENVELOPE'}
  if($status-ne 200){
    try{
...
$carrier = $carrier.Replace("'HOST06_REVIEW_READY|ordinal='+`$ordinal", "'HOST06_REVIEW_READY|ordinal=C1'")
$carrier = $carrier.Replace("'\AHOST06_REVIEW_V1\|ordinal='+`$ordinal+'\|decision=", "'\AHOST06_REVIEW_V1\|ordinal=C1\|decision=")
...
$carrier = $carrier.Replace('Work Order: FRWO-005-v7', 'Work Order: FRWO-005-v10').Replace('Shell: FRSH-005-v1 through FRSH-005-v1-VR-11', 'Shell: FRSH-005-v1-VR-41').Replace('Consumed ordinals: "+$(if($ordinal-eq 2){''2 accepted''}else{''2 objective rejection; 3 accepted''})+"', 'Consumed attempt: C1 accepted').Replace('Selected attempt: attempt-0"+$ordinal+".png', 'Selected attempt: attempt-C1.png')
...
$carrier = $carrier.Replace("'HOST06_PRODUCTION_COMPLETE|ordinal='+`$ordinal", "'HOST06_PRODUCTION_COMPLETE|attempt=C1'")

if($carrier.Contains('foreach ($ordinal in @(2,3))') -or $carrier.Contains('attempt-02') -or $carrier.Contains('attempt-03')){throw 'C1_PATCH'}
if(-not $carrier.Contains('HorizonArchive.Host06V10.StrictJson') -or -not $carrier.Contains('.attempt-C1-7eb25013-d4b3-46d9-ac8e-c49fd7242161.stage')){throw 'PARSER_PATCH'}
