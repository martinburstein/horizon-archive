...
"@
  ...
  $production.Stage='PT12_MATERIALIZE_STAGE'
    ...
    [Console]::Out.WriteLine('HOST06_REVIEW_READY|ordinal=C1'+'|length='+$targetIdentity[4]+'|sha256='+$decodedSha)
    ...
    $decisionPattern='\AHOST06_REVIEW_V1\|ordinal=C1\|decision=(?<decision>ACCEPT|REJECT)\|technical=true\|physical=(?<physical>true|false)\|layouts=(?<layouts>true|false)\|accessibility=(?<accessibility>true|false)\|codes=(?<codes>NONE|PHY(?:-\d{2})(?:,PHY-\d{2})*|LAYOUT|ACCESSIBILITY)\n\z'
    ...
    $provenance="# HOST06 Source Provenance`n`nWork Order: FRWO-005-v10`nShell: FRSH-005-v1-VR-41`nHelper: HOST06-FILE-IDENTITY-PSNET-v1`nHelper source: 1693 / 98cf564b7d22da686adc204a3f6051927ac2d37ef7b5f2fe22d774cb10d5da97`nHelper DLL: "+$dllLength+" / "+$dllSha+"`nTransport: HOST06-IMAGE-API-PSNET-v1`nEndpoint: https://api.openai.com/v1/images/generations`nModel/options: gpt-image-2 / n=1 / 3840x2160 / high / opaque / png`nPrompt: HOST06-GEN-PROMPT-v1 / no input`nConsumed attempt: C1 accepted`nSelected attempt: attempt-C1.png`nSelected identity: "+$targetIdentity[4]+" / "+$decodedSha+"`nTechnical/physical/layout/accessibility: pass / pass / pass / pass`nManifest: c7ca95201029b490f2460a846e3dc2a64a26775b57e8c587cbc2d874df654d99`nProduct: Visual Direction/Production Masters/2026-08-10-first-run-host06/host06-stranded-lens-cradle-master-v1.png`nAccepted media unchanged: true`nHelper/live cleanup: complete`n"
    ...
    [Console]::Out.WriteLine('HOST06_PRODUCTION_COMPLETE|attempt=C1'+'|length='+$targetIdentity[4]+'|sha256='+$decodedSha+'|helperRootAbsent=true|liveRootAbsent=true')
    break
} catch {
