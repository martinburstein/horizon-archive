...
try{
  ...
  if($carrierBytes.Length-ne 34795-or([BitConverter]::ToString($sha.ComputeHash($carrierBytes))).Replace('-','').ToLowerInvariant()-cne'4f6d7faa80214a3753cae5f49c5fb3e393c9675c8feaab7b8a4d3ab15cd03c68'){throw 'PARENT_CARRIER_IDENTITY'}
  foreach($byte in $carrierBytes){if($byte-gt 127){throw 'PARENT_CARRIER_ASCII'}}
  $carrier=$strictUtf8.GetString($carrierBytes)
  if($carrier.Length-ne 34795){throw 'PARENT_CARRIER_LENGTH'}
  ...
  if($launcherBytes.Length-ne 2639-or$launcherHash-cne'42a1f80bc9ad18fa4fd8b9151028333bfa97fccfd4aebdf95268222a040b2b06'){throw 'PARENT_LAUNCHER_IDENTITY'}
