...
additional Host 06 source. `FRWO-005-v3` supersedes `FRWO-005-v2` by replacing
the unworkable returned-path/managed-cleanup transport with one strict direct
PNG data URL, one decode into fresh GUID OS temp, payload/temp byte-and-SHA
identity, exact rejected/selected temp cleanup, and explicit non-gating opaque
managed-residual accounting. Ordinal 1 remains consumed; at most ordinals 2
and 3 remain; and no more than three total ordinal-associated managed residuals
may exist. The exact next stage is one fresh Science revalidation of
`FRWO-005-v3`; no Mission, generation, implementation, reveal, or later-
...
| ---: | --- | --- | --- |
| 1 | `FR-03 / Host 06` | **SELECTED - `FRWO-005-v3`**: one new source plus fixed Host 05 -> Host 06 -> unchanged `L02-03` integration | First missing fixed encounter and exact next learning adjacency after Host 05; the revised Work Order freezes strict PNG data-URL ingress, exact temp identity/cleanup, and bounded opaque managed-residual treatment for fresh Science review while the old-source HOLD remains exact |
| 2 | `FR-03 / Hosts 07-15` | Remaining Drowned movements and pedestrian/Crown sequence | Blocked behind independent acceptance or HOLD of `FRWO-005-v3`; no skip or repeatable host pattern is authorized |
...
`FRWO-005-v3` owns only one new eventual canonical Host 06 Drowned source and
...
not a silent amendment, of superseded `FRWO-005-v2`.
...
source envelope; the maximum-three-ordinal built-in-only strict PNG data-URL
firewall; one decode and payload/temp byte/SHA identity in fresh GUID OS temp;
exact rejected/selected temp cleanup; the explicit allowance of at most one
zero-authority opaque `$CODEX_HOME` managed residual per consumed ordinal and
three total; the pre-generation Combat manifest for exactly seventeen literal
...
outside the workspace with zero authority. No `output_hint` parsing, CLI/API
fallback, managed-parent discovery or cleanup, accepted-media pixel inspection,
...
`FRWO-005-v3` and do not revise this historical finding.
...
Operations selected then versioned the distinct successor as `FRWO-005-v3`.
...
`FRWO-005-v3` is selected and one fresh Science review is the exact next
