...
| 2 | `RP-002` | `Civic Record Encounter — City Beneath` | `RP-001 anchor complete -> civic route available` | `IN DESIGN — A5 PARTIAL / REVISE` | `NOT READY / NON-ROUTABLE` | `LORE LOCKED — EXECUTABLE LITERACY + HUMAN RAI + ATOMIC SAVE + FIVE-OBSERVATION + RESPONSIVE INTERFACE + HOTSPOT OWNERSHIP/ACTIVATION + ROUTE-INTEGRATION + ROUTE-TO-OBSERVATION + FIRST-NEAR-DISPATCH MEANING` | `A2 ROUTE-INTEGRATION + ROUTE-TO-OBSERVATION + FIRST-NEAR-DISPATCH PRESENTATION LOCKED; A3 PRESERVATION PASS` | `PY-009 SOLIDIFIED — NO REOPEN; OBSERVATION/ROUTE/HANDOFF/PRESENTATION CROSS-CREDIT PROHIBITED` | `RP002-RAI-01 SOLIDIFIED — NO REOPEN; OBSERVATION/ROUTE/HANDOFF/PRESENTATION CROSS-CREDIT PROHIBITED` | `A4 FIVE-OBSERVATION + ATOMIC SAVE + RESPONSIVE INTERFACE + HOTSPOT DISPATCH + ROUTE-INTEGRATION + ROUTE-TO-OBSERVATION + FIRST-NEAR-DISPATCH GRAPHS LOCKED; ISOLATED A5 FIRST-DISPATCH ORCHESTRATION NEXT` | `PROTECTED TOUR + PREREQUISITES + PY/RAI + FIVE-OBSERVATION + ATOMIC SAVE + RESPONSIVE COMPONENT + HOTSPOT GEOMETRY/DISPATCH + ISOLATED ROUTE-STATE + ROUTE-TO-BLANK-OBSERVATION ORCHESTRATION COMPLETE; BROWSER STORAGE/ART/NORMAL EXPOSURE ABSENT` | A4 locked only the pure first-dispatch graph from exact zero-ID `SC-03-10`: any one of three equal near actions, resolved once through existing hotspot authority, may produce exactly its matching model-owned ID and Scene statement, System 1-of-3 status, and zero-credit evidence return. Seven-modality parity, all six near orders, one-hit/fail-closed unsafe or Tour input, heading-first focus, first-incomplete recovery/resume, separate route return, privacy, responsive/accessibility state, and no replay are exact. Both SOLIDIFIED chains, later observation/save/atomic contracts, accepted/Tour bytes, world/clocks, continuation, null city delta, closed identity, no successor, and zero city/access/external action remain unchanged. A5 implements only the isolated pure orchestration seam. No code, copy, geometry, storage, art, route exposure, accepted state, successor, Demo, or Working change occurred in A4. |
