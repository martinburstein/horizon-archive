#!/usr/bin/env python3
"""Validate TD-011 aggregate production output against PBA-TD011-v1."""
...
PBA = ROOT / "Production Pipeline" / "Skyscraper Test Drives" / "TD-011" / "04A-PRODUCTION-BUDGET-AUTHORITY.json"
PREDECESSOR_PBA = ROOT / "Production Pipeline" / "Skyscraper Test Drives" / "TD-010" / "04A-PRODUCTION-BUDGET-AUTHORITY.json"
...
def fail(message: str) -> None:
    raise SystemExit(f"PBA-TD011-v1 FAIL: {message}")
...
def main() -> None:
    ...
    predecessor = json.loads(PREDECESSOR_PBA.read_text(encoding="utf-8"))
    caps = authority["caps"]
    if not DIST.is_dir():
    ...
    accepted_hashes = {item["sha256"].upper() for item in predecessor["basis"]["runtimeMedia"]}
    ...
    if args.mode == "baseline":
        ...
        if len(javascript) != 1 or javascript[0].stat().st_size != basis["javascript"]["bytes"] or sha256(javascript[0]) != basis["javascript"]["sha256"]:
            fail("baseline JavaScript identity differs from accepted TD-010")
        if len(css) != 1 or css[0].stat().st_size != basis["css"]["bytes"] or sha256(css[0]) != basis["css"]["sha256"]:
            fail("baseline CSS identity differs from accepted TD-010")
