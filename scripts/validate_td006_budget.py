...
def main():
    ...
    accepted_media = {
        item["sha256"].upper(): item for item in basis["runtimeMedia"]
    }
    present_media_hashes = {item["sha256"] for item in media}
    new_media = [
        item for item in media if item["sha256"] not in accepted_media
    ]
    ...
    missing_accepted = set(accepted_media) - present_media_hashes
    if missing_accepted:
        errors.append(
            f"accepted runtime media missing: {len(missing_accepted)} hash(es)"
        )
    for digest, expected in accepted_media.items():
        matches = [item for item in media if item["sha256"] == digest]
        if len(matches) == 1 and matches[0]["bytes"] != expected["bytes"]:
            errors.append(f"accepted media byte mismatch: {expected['name']}")
    disallowed = [
        item["name"]
    ...
    if args.mode == "baseline":
        ...
        if len(media) != len(basis["runtimeMedia"]):
            errors.append("baseline runtime-media file count mismatch")
